...
Rectangle {
    ...
    ArchivedMatterEntryGuardDialog {
        id: archivedMatterDisbGuard
        t: root.t
        appRef: root.appRef
        entryKind: "disbursement"
        onEntryConfirmed: root.saveDisbursementEntry(true)
    }
    ...
    function clearForm() {
        ...
        taxExemptCombo.editText = "Taxable (HST 13%)"
        billClaimPctInput.text = "100"
    ...
    function saveDisbursementEntry(skipArchivedGuard) {
        var selectedMatter = findMatter(matterCombo.editText)
        if (!selectedMatter || !String(selectedMatter.matterId || "").trim().length) {
            setFeedback("Select an existing matter before saving a disbursement.", true)
            return false
        }
        ...
        if (selectedMatter.active === 0 || selectedMatter.active === false
            ...
            if (matterStatus === "archived" && !skipArchivedGuard) {
                archivedMatterDisbGuard.openFor(selectedMatter)
                setFeedback("Matter is archived. No disbursement was saved.", true)
            } else {
        ...
        var amount = parseAmount(amountInput.text)
        if (!isFinite(amount) || amount <= 0) {
            setFeedback("Enter a positive disbursement amount.", true)
            return false
        }

        var billPct = parseAmount(billClaimPctInput.text)
        if (!isFinite(billPct) || billPct < 0 || billPct > 100) {
            setFeedback("Bill Claim % must be between 0 and 100.", true)
            return false
        }

        if (!root.appRef || !root.appRef.saveDisbursementDocketEntry) {
            setFeedback("Disbursement saving is unavailable.", true)
            return false
        }

        var taxExempt = normalized(taxExemptCombo.editText).indexOf("exempt") >= 0 ? 1 : 0

        var result = ({})
        try {
            result = root.appRef.saveDisbursementDocketEntry({
                ...
                "taxExempt": taxExempt,
                "billPct": billPct,
                "description": String(descriptionInput.text || "").trim(),
        ...
        if (result && result.ok && result.verifiedExact) {
            ...
            setFeedback("Disbursement saved" + (entryId.length ? ": " + entryId : "") + ".", false)
            root._hydrating = true
            amountInput.text = ""
            descriptionInput.text = ""
            root._hydrating = false
        ...
        setFeedback(String((result && result.message) || "Disbursement could not be saved."), true)
        return false
    }
    ...
    ColumnLayout {
        ...
        Text {
            ...
            text: "Record an out-of-pocket matter expense (court filing, corporate search, courier, travel) directly into invoiceable WIP."
            color: root.mutedInk
            font.pixelSize: 13
        ...
        GridLayout {
            ...
            ModernComboBox {
                ...
                metrics: root.metrics
                label: "Tax Treatment"
                fullModel: ["Taxable (HST 13%)", "Tax Exempt"]
                editText: "Taxable (HST 13%)"
                Layout.fillWidth: true
                Layout.columnSpan: 3
                Layout.preferredHeight: 52
                z: 1
            ...
            Text {
                ...
                text: "HST is calculated automatically when the disbursement is invoiced. Mark 'Tax Exempt' for items not subject to HST (e.g., court filing fees)."
                color: root.mutedInk
                font.pixelSize: 12
                wrapMode: Text.WordWrap
            }
            ...
            ScrollView {
                ...
                TextArea {
                    ...
                    bottomPadding: 10
                    color: root.ink
                    placeholderText: "e.g., Court filing fee, corporate search, courier, travel expense…"
        ...
        RowLayout {
            ...
            PillButton {
                ...
                metrics: root.metrics
                sfxBus: root.sfxBus
                text: "Save Disbursement"
                primary: true
                Layout.preferredWidth: 200
                Layout.preferredHeight: 44
                onClicked: root.saveDisbursementEntry()
            }
        }
    }
