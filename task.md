...
- [x] Rebuilt the complete candidate-validation executable at `dist\CSPM\CSPM.exe` and deployed the same package to `C:\Programs\CSPM\CSPM.exe` (matching SHA-256 `20965F32BC0DBD89B82E87F65AFCA7C0759ECC079D8B723D75BA459FD054A3FD`; 4,325 build files). Installed Payment Entry and collection-dialog QML match source byte-for-byte. The prior dist package is recoverable at `to_delete\dist__replaced_release_20260910_100706`.
- [x] Outside-sandbox Qt/WebEngine startup validation passed from both the built candidate and the deployed installed path. The installed app reached first main-window pixel in 13.422 seconds, remained responsive, and closed through the normal transition. No payment was posted; shutdown found local/cloud data already matching and released the shared checkout marker.
