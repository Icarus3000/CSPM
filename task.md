...
- [x] Push the repair to `origin/wip/ap-defect-validation` (`c5e1fd7`), build/deploy the governed candidate-validation package, and validate the installed executable. Dist/installed EXE SHA-256 values match at `6926332C350F8D50C1423C9FAD6B143FED6F8463D76A21DA39B0492F59B5E4E6`; both package trees contain 4,361 files and their Invoice Builder QML matches source exactly. The replaced dist package is recoverable at `to_delete\dist__replaced_release_20260924_145913`.
- [x] Outside-sandbox installed Qt/WebEngine startup passed: `C:\Programs\CSPM\CSPM.exe` reached first pixel and input-ready, closed normally with exit code 0, published an unchanged workbook package, and released the shared checkout. Local/cloud workbook hashes match and no financial record was written.
...
- [x] Commit and push the invoice repair to `origin/wip/ap-defect-validation` (`9f7b88c`), then build and deploy the governed candidate-validation package. Dist and installed executables match at SHA-256 `EB31CF24D8E489BDC1B1884893169BA39EB104082F1A47B09D035A12DB365EF0`; both package trees contain 4,361 files and the bundled invoice QML/template assets match source exactly. The replaced dist package is recoverable at `to_delete\dist__replaced_release_20260924_101903`.
- [x] Outside-sandbox installed Qt/WebEngine validation passed: `C:\Programs\CSPM\CSPM.exe` reached first main-window pixel and input-ready, closed normally with exit code 0, published an unchanged workbook package, and released the shared checkout. Local/cloud `CSPM.xlsm` and `Dockets.xlsm` hashes match; no financial record was written.
...
- [x] Commit and push the repair to `origin/main` (`bf12768`), build the governed candidate-validation package, and replace `C:\Programs\CSPM\CSPM.exe`. Dist/installed executables match at SHA-256 `C2DA86AE33A938FC796EF46FEC97A7DE6B0F7305B4ADBB792BB90E2FF865AA16` with 4,274 files in each package tree; the prior release is recoverable at `to_delete\dist__replaced_release_20260910_144425`.
- [x] Outside-sandbox installed Qt/WebEngine startup validation passed: the application reached `ready-to-reveal` and first main-window pixel, remained responsive, and exited normally. Shutdown confirmed cloud authority and the local workbook replica were already identical and released the shared checkout marker; no financial record was posted or modified.
...
- [x] Scope the collection list with a dedicated **Client** selector instead of a combined invoice/client/matter text filter. Its options are the distinct work clients represented by the selected billing client's open invoices, with **All clients** first; selection uses an exact case-insensitive client match and updates the visible-row count and shown A/R total.
- [x] Fix Apply-field focus loss: typing now mutates the existing allocation row instead of replacing the complete ListView model on every keystroke; the focused delegate remains current and is kept within the visible viewport.
...
- [x] Sandbox-safe validation: the focused and adjacent payment/invoice suite passed (**37 passed**); the revised dialog compiled `Ready`; governed QML lint reported no errors; an offscreen QML smoke exercised exact client filtering; and `git diff --check` passed with line-ending notices only.
- [x] Rebuilt the complete candidate-validation executable at `dist\CSPM\CSPM.exe` and deployed the same package to `C:\Programs\CSPM\CSPM.exe` (matching SHA-256 `4ED700C3F772F39FD75CED69388E64206350992066A89AFA8830F7327A805F31`; 4,325 files in both package trees). Source, dist, and installed collection-dialog QML match at SHA-256 `3067680921323FFFB099327A61CBF96CE0E63D5F7F5C4CF0CC7AB5D8CFF9E314`. The preceding dist package is recoverable at `to_delete\dist__replaced_release_20260910_110311`.
- [x] Outside-sandbox Qt/WebEngine startup validation passed from the updated installed path. The app reached first main-window pixel in 14.240 seconds, remained responsive, and closed through the normal transition. No payment was posted; shutdown published an unchanged package and released the shared checkout marker.
- [ ] Manual application acceptance: open **Payment & Adjustment Entry** → **Collect billing-client A/R**; choose a multi-invoice billing client, then choose `Gold` from the **Client** selector and confirm only that exact client's invoices remain while the billing client stays unchanged. Type a multi-digit/decimal amount continuously in Apply and confirm focus/scroll position remain stable. Then, against a recoverable test workbook, balance and post a receipt and verify one transaction plus matching per-invoice ledger/payment-history entries.
...
- [x] Proved the apparent divergence was metadata-only: all 80 OOXML package entries matched except `docProps/core.xml`, whose sole difference was the `dcterms:modified` value (`2026-09-07T23:05:06Z` local versus `2026-09-07T23:05:07Z` cloud). Worksheets, relationships, VBA, and all other workbook payloads were byte-identical.
- [x] Added a narrow OOXML payload comparison to `SyncService`. It ignores only the volatile modified-at element after verifying every package entry; a proven metadata-only mismatch is aligned to the exact cloud workbook with a verified recovery copy and a new full-file SHA-256 common ancestor. Any substantive entry difference remains a hard conflict that preserves both files.
- [x] Added Legacy Dockets write preflight in Python and QML so both full and filtered imports verify the active write lease before opening progress UI or beginning long-running import work. The repository guard remains authoritative at save time.
- [x] Reconciled the actual metadata-only mismatch through the governed service. The former local workbook is recoverable at `C:\Users\cschn\AppData\Local\CSPM\data\sync_recovery\CSPM_before_metadata_alignment_20260910_071639_8fe40e7b.xlsm`; local/cloud workbook hashes now match exactly and no checkout marker remains.
- [x] Fixed packaged deployment cleanup so stale empty Python package directories are removed deepest-first without recursive deletion and while preserving `data`, `backups`, and `logs`. This removed an empty legacy `numpy` namespace directory that caused OpenPyXL to fail on `numpy.short` in the first packaged validation attempt.
- [x] Sandbox-safe validation: Python compilation passed; focused checkout, metadata-conflict, import-preflight, safe-selection, reconciliation, and deployment-cleanup tests passed (**20 passed**); governed QML lint completed with existing warning-only diagnostics; `git diff --check` passed with line-ending notices only.
- [x] Outside-sandbox packaged Qt/WebEngine validation: deployed `C:\Programs\CSPM\CSPM.exe` acquired a writable checkout (`Cloud authority and local replica already match`) and reached first main-window pixel. The agent-started process was stopped, its dead validation lease was audit-recovered, and the fresh lease was released. Dist/installed SHA-256 is `502B6F44CFBCB9D71273EE6F5523F1209A455FB2C1A4082C3B7100B16EED765B`; no `Zone.Identifier` or stale NumPy directory remains.
- [ ] Manual application acceptance: launch CSPM normally and repeat the reviewed six-row `88 queen` Legacy Dockets import. Confirm it completes without the read-only failure. No import was executed automatically during this repair.
...
- [x] Diagnosed root causes:
  - Vector artwork in `assets/CS.svg` and `src/qml/assets/CS.svg` had a `0 0 1536 1536` viewBox, but actual non-empty artwork was bounded between Y=309.39 and Y=1197.31, leaving ~338px (22.1%) of empty bottom padding inside the SVG asset itself.
  - In `ReportWindow.qml`, `reportLogo` used a square 1:1 box with default `verticalAlignment: Image.AlignVCenter`, floating the horizontally oriented monogram ~18% above the contact text floor.
  - In `report_pdf_exporter.py`, `_draw_header_footer` called `canvas.drawImage` with `anchor="c"` (center) and square/misaligned default bounding dimensions, keeping the logo elevated instead of resting flush at `contact_bottom`.
  - In HTML invoice templates (`Concept_A2.html`, `Standard_Elite.html`, `LIHDC_Format.html`, `Concept_A.html`, `Concept_A_Agency.html`, `Concept_B.html`), logo containers and brand headers used `align-items: center` or unadjusted square viewBoxes.
- [x] Executed full-stack bottom-floor alignment:
  - Tightened SVG asset viewBox to `60.8 309.4 1405.4 887.9` (matching exact artwork geometry with 0px transparent margin) across `assets/CS.svg` and `src/qml/assets/CS.svg`.
  - Updated HTML invoice templates:
    ...
    - Set header rows and firm details to `align-items: flex-end` so the CS logo artwork baseline rests directly on the same bottom floor as the contact text block.
  - Updated `src/qml/components/ReportWindow.qml`:
    ...
    - Set `width: Math.round(height * 1.583)` and `verticalAlignment: Image.AlignBottom`, `horizontalAlignment: Image.AlignLeft`.
  - Updated `src/python/services/report_pdf_exporter.py`:
    - Computed precise contact descent floor `contact_bottom = (top - 11) - (num_contact_lines - 1) * 8.5 - 2.0`.
    - Computed `contact_block_height = (top + 9.5) - contact_bottom` and proportional default width `logo_w = round(logo_h * 1.583, 2)`.
    - Set `logo_y = contact_bottom` and `anchor="sw"` in `canvas.drawImage`, guaranteeing the bottom-left corner of the logo artwork anchors flush with `contact_bottom`.
- [x] Validated:
  - `python -m py_compile src/python/services/report_pdf_exporter.py` passed.
  - `scripts/qmllint.ps1 src/qml/components/ReportWindow.qml` returned 0 errors.
  ...
  - All 25 unit tests across statements, reports, and invoices passed.
- [x] Built and deployed release executable to `C:\programs\CSPM\CSPM.exe`.
...
- [x] Corrected the prior log inference. The missing Professional start/progress/settled lines were debug-level telemetry suppressed unless verbose logging is enabled; the warning-level staging marker alone did not prove that the old 240 ms timeline failed to start. The remaining stutter was architectural: CSPM changed its layered native host envelope around a QML texture transition, while Source left the entire state transaction with DWM.
- [x] Added a Professional-main-window-only native style bridge. Qt's client-drawn frameless chrome remains, while the HWND replaces `WS_POPUP` with `WS_CAPTION`, `WS_THICKFRAME`, system menu, minimize, and maximize style bits and receives one `SWP_FRAMECHANGED`. Console and detached windows retain their prior path.
- [x] Replaced the accepted Professional button path with true HWND `SW_MAXIMIZE` / `SW_RESTORE` requests. QML now follows `Window.Maximized` / `Window.Windowed`, updates the glyph/model/persistence after the native event, preserves the native normal bounds, and leaves `maximizeAnimInProgress`, the temporary scene-graph layer, manual monitor-envelope staging, and the 240 ms Professional timeline out of the normal path. The prior texture path remains only as a bridge-unavailable compatibility fallback.
- [x] Returned Professional `Win+Shift+Arrow` to Windows instead of consuming it in CSPM's two keyboard hooks. The native maximized HWND and Windows-owned normal placement now move together; QML adopts the resulting screen/state event.
- [x] Sandbox-safe validation passed: Python compilation for the changed runtime modules; **9 passed** across maximize/restore choreography, native style, and main-window layout persistence; governed `scripts/qmllint.ps1` parsed `DetachedShellWindow.qml` with no syntax error and only its existing warning-level diagnostics; scoped `git diff --check` passed.
- [x] Outside-sandbox source runtime validation passed through `launch.ps1`: the real Qt/WebEngine shell reached a responsive main window; startup logged the bridge as ready; the live custom-chrome HWND was `0x16CF0000` / `0x00080100`; CSPM's own maximize glyph entered true `IsZoomed` / `0x17CF0000` state; its restore glyph returned to the exact prior normal HWND rectangle and `0x16CF0000`. The app is left open in normal state for foreground review.
- [ ] **Manual visual acceptance remains the blocking P0 gate:** from the open smaller Professional window, click Maximize/Restore ten times and confirm Source-equivalent continuity with no pre-jump, mask pop, blank frame, duplicate motion, or layout snap. Then test Maximize → `Win+Shift+Arrow` → Restore and confirm Windows restores on the monitor that owns the native maximized window. Do not advance to another visual state machine until Cory accepts this real-app motion.

## Accepted Packaged Startup Hotfix Integration (2026-08-30)

- [x] Recovered the interrupted release state before changing it. The failed packaged run reached `briefing-snapshot-ready` at 86% and then failed while compiling the hidden shell: `InvoiceBuilderView.qml:1943` assigned `background` to a plain `Text`. The deterministic `Component.Error` entered all 180 retries from t+10.446 s through t+78.871 s, exhausted at t+78.874 s, and never reached `ready-to-reveal` or a first pixel. The roughly 2.1 s synchronous component compile did not explain the long splash; the invalid QML plus repeated retries did.
- [x] Classified the recovered `d4196fc` operation as **B — candidate output completed, but canonical finalization/validation did not finish**. Retained session evidence showed both PyInstaller COLLECT outputs completed and the full 4,358-file tree remained under its staging name after the builder exited; the prior agent moved that complete tree as one directory to the candidate path and was interrupted immediately before promotion. No individual-file overlay occurred, no builder remained active, and no process was terminated during recovery.
- [x] Reviewed `7fda5e9ae89d2b648859edc659696c9941b7487b`: `src/python/main.py`, `src/qml/BootstrapRoot.qml`, and `tests/test_startup_shell_failure_contract.py`. It makes shell compilation asynchronous, sends deterministic component errors through an explicit diagnostic/quit path instead of the 180-retry path, preserves the hidden native-splash handoff and mandatory initialization, and adds no silent fallback or unrelated visual work. It shortens deterministic failure but does not repair the observed invalid QML by itself.
- [x] Reviewed the required root-cause follow-up `d4196fcfb3d6af0b2ae396ff8d5fa474a18c16da`: it removes only the invalid `Text.background` assignment from `InvoiceBuilderView.qml` and adds a focused Custom Fee QML regression assertion. The accepted Custom Fee implementation remains inherited unchanged from `dce3112`; no Custom Fee behavior, workbook schema, visual state machine, or conflict-checking work was broadened.
- [x] Sandbox-safe validation passed: `py_compile src/python/main.py`; governed `scripts/qmllint.ps1` on `BootstrapRoot.qml` and `InvoiceBuilderView.qml` (exit 0, existing warning diagnostics only); `git diff --check`; canonical package structural validation; exact source-versus-package QML hashes; and 38 applicable startup/splash/briefing/restart/maximize/Custom Fee tests. The one additionally selected failure is the already documented stale focus assertion against the separate tray-toast `WindowStaysOnTopHint`; the QML and test are unchanged from `ec9b7a3` and are not a hotfix regression.
- [x] Independently validated the complete `release_candidate_custom_fee_startup_hotfix_20260830_d4196fc` package: 4,358 files / 678,617,490 bytes / tree SHA-256 `343F43A3AC28C9DB5DF1553983C0178AC957A81F38D86BCBA51ECE5F33AF5323`; `CSPM.exe` SHA-256 `1E983211BB589D41E4D90EC415552BCC4073C5F9B2B3D589F06E7B7800206DE0`. Main/recovery executables, splash assets, governed templates, QML, Qt Quick/QML libraries, WebEngine process/DLLs/resources/locales, and ICU data are present; no incomplete marker or alternate data stream remains.
- [x] Promoted that exact candidate through `scripts/promote_verified_release_package.py`. The source and post-promotion manifests match exactly. The immediate prior `7fda5e9` fail-fast-only package is preserved at `to_delete/dist__manual_replaced_release_20260830_163733`, its predecessor `dce3112` remains at `to_delete/dist__manual_replaced_release_20260830_154035`, and the successful promotion audit is `to_delete/release_promotion_20260830_163733.json`.
- [x] Workbook safety remained intact: `data/CSPM.xlsm` stayed byte-identical at SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`; canonical integrity remains 12 tables / 1,537 rows / 0 errors / 7 unchanged unrelated warnings. Those warnings were not addressed. Conflict checking remains inactive.
- [x] Cory completed the required outside-sandbox packaged-startup acceptance with `dist/CSPM/CSPM.exe` at 16:45 on 2026-08-30. The accepted `d4196fc` package advanced from the native splash through `ready-to-reveal`, completed the native-to-QML handoff, displayed a usable main window, opened a workspace, and closed normally. The fresh frozen runtime log contains no former 180-retry cycle, startup diagnostic, component failure, or stuck-splash state. This acceptance is startup-only; it does not claim that Cory repeated the remaining Custom Fee lifecycle scenarios in the corrected package.
- [x] Exact integrated-source validation: the eight focused startup/splash/briefing/restart/focus/maximize/Custom Fee files collected 39 tests, with **38 passed and 1 known pre-existing failure**. The sole failure is the stale tray-toast `Qt.WindowStaysOnTopHint` focus assertion; both implicated files are unchanged by this hotfix and the same test fails at parent `ec9b7a3`. `py_compile src/python/main.py`, governed QML lint for both affected QML files, and both `git diff --check` scopes passed. QML lint retained existing warning-level diagnostics only.
- [x] Package/source correspondence remains exact: bundled `BootstrapRoot.qml`, `DetachedShellWindow.qml`, and `InvoiceBuilderView.qml` match integrated `d4196fc` Git blobs. The accepted EXE remains SHA-256 `1E983211BB589D41E4D90EC415552BCC4073C5F9B2B3D589F06E7B7800206DE0`; the promoted tree remains 4,358 files / 678,617,490 bytes / SHA-256 `343F43A3AC28C9DB5DF1553983C0178AC957A81F38D86BCBA51ECE5F33AF5323`. No rebuild was required because integration did not change runtime source beyond the already accepted lineage.
- [x] Integrated the accepted lineage without touching the dirty main worktree: a clean temporary branch fast-forwarded from `ec9b7a3d3b944c2c3d48d5abdb67df09cb6a6694` through `7fda5e9ae89d2b648859edc659696c9941b7487b`, documentation checkpoint `14e37772dff7b004400cb36cf3ea2c9f412fe6fb`, and root-cause correction `d4196fcfb3d6af0b2ae396ff8d5fa474a18c16da`. The source push fast-forwarded `origin/main` from `ec9b7a3` to `d4196fc`; this entry is the sole follow-up documentation reconciliation. No hotfix commit was amended or squashed, and no force push, stash, reset, worktree removal, release cleanup, or package replacement occurred.
- [x] Post-validation workbook hashes are unchanged. Canonical integrity remains **12 tables / 1,537 rows / 0 errors / 7 unchanged warnings** at repository workbook SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`. The warnings were deliberately not addressed, Custom Fee behavior was not broadened, and conflict checking remains inactive.
- [ ] **Next development priority (separate assignment):** implement the proper conflict-checking workflow. Do not activate A08 or begin conflict tables, matching, review, PDFs, history, SQLite, or SQL until that work is explicitly started.

## Invoice Builder Custom Fee Idempotency and Ownership Repair (2026-08-30)

- [x] Replaced the unsafe Add Custom Fee append path with one request-idempotent command. Each deliberate dialog opening receives a distinct opaque request ID; the service derives a deterministic custom-fee line ID from stable `DraftID + RequestID`, serializes draft lifecycle commands, recognizes repeat/concurrent calls, and verifies uncertain writes by read-back. Identical amounts/descriptions remain valid when deliberately submitted under different request IDs.
- [x] Kept the existing workbook schema. `DraftInvoices.DraftID`, `tblTimeEntries.EntryID`, a real existing `MatterID`, draft state fields, and structured `LockAudit` markers (`EntryType`, `FeeOrigin`, `DraftOwnerID`, `DraftRef`, `RequestID`, `CustomFeeLineID`, `CustomFeeState`, and final invoice) provide sufficient durable ownership and identity. No schema, workbook, migration, or conflict-checking change was made.
- [x] Enforced the lifecycle boundary: custom fees use a valid matter already represented by draft time or disbursement work; reopen/save/edit preserves stable identity; remove hard-deletes only the exact exclusively draft-owned fee; ordinary dockets return to unbilled WIP; abandoning a draft destroys its custom fees and releases only ordinary work. Financial references/status/payment/reissue values block fee edit/removal/draft deletion before any write.
- [x] Made finalization one serialized atomic financial command. Draft custom fees are validated before billing, marked `Finalized` with the final invoice reference, and never released as ordinary WIP. A lost completion acknowledgement is recovered only when the draft is gone, every custom-fee footprint is consistently billed, and exactly one Receivable, Invoice Log, and Revenue ledger record exists; retry cannot add duplicate financial rows.
- [x] Routed Add Custom Fee through a background controller worker. QML now guards rapid double-click/repeated Enter with a pending state; the controller suppresses duplicate in-flight request IDs; backend idempotency remains authoritative. The shared Console/Professional dialog retains the same request ID after an actionable failure for safe retry and creates a new ID only for a new deliberate opening.
- [x] Added `tests/test_invoice_custom_fee_lifecycle.py` covering same-request replay, distinct identical fees, concurrent duplicate calls, pre-write and post-write uncertainty, disbursement-only matter ownership, edit/reopen identity, exact removal, ordinary-WIP release, draft abandonment, financial-dependency refusal, exactly-once finalization, mismatched retry refusal, lost finalization acknowledgement, clean retry after pre-write failure, and the QML/controller contract.
- [x] Sandbox-safe validation: Python compilation passed; focused and adjacent invoice suites passed **32 tests in 11.25s**; governed QML lint parsed both Invoice Builder QML files with no syntax error (the repository's existing warning-level diagnostics remain); `git diff --check` passed. Canonical workbook integrity remains **12 tables / 1,537 rows / 0 errors / 7 unrelated warnings**, SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`.
- [x] Cory completed outside-sandbox source-launch acceptance with `.\launch.ps1`: in Invoice Builder on a safe draft with a real client and matter, Cory entered a test description/amount, activated Add rapidly more than once, and observed exactly one custom-fee line with the intended behavior. This acceptance is deliberately limited to the reported rapid-activation path.
- [ ] Further manual lifecycle acceptance remains: edit/reopen, remove, abandon, dependency-blocking, finalization/retry, and Console-versus-Professional comparison. Do not infer those checks from Cory's rapid-activation acceptance.
- [x] Release checkpoint: committed Custom Fee-only source as `dce31122d44ca3455a292382f607affd96cfa20c` (`Harden invoice draft custom fee workflow`) and pushed `main` to `origin/main`. The exact commit passed Python compilation and all 14 Custom Fee lifecycle tests. Its selected 38-test adjacent suite produced 37 passes and one pre-existing, out-of-scope LIHDC total assertion failure (`316.00` versus expected `221.20`); no LIHDC/discount repair was added to this checkpoint.
- [x] Exact-source package validation: governed QML lint completed with warning-level diagnostics only and no syntax failure. Workbook integrity remains SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`, 12 tables / 1,537 rows / 0 errors / 7 unchanged unrelated warnings.
- [x] Canonical PyInstaller build completed from the clean detached `dce3112` commit via `C:\Projects\__CSPM\.venv_CORY_CorySchneider\Scripts\python.exe scripts\build_release.py --validate`. Verified package manifest: 4,358 files / 678,615,222 bytes / SHA-256 `0EC666DA28A8134FC2299D44B149AD2CDF14ACB13C2F426861232B5DDF80CD24`; `CSPM.exe` SHA-256 `9C7379024912E154E39F42528D58E1CDEC043E87A595D66EBBC5D36DEAFB36A6`.
...
- [x] Classified `data/CSPM.xlsm` as a divergent repository snapshot, not the authoritative live workbook. Read-only preflight proved exact record cardinality and field shape, exactly two permitted matter dependents, no invoice/payment/credit/adjustment/transaction/ledger dependency, no protected-invoice overlap, absence from all configured live/cloud CSPM and Dockets workbooks, and presence of both time-entry IDs in immutable historical evidence.
- [x] Added `scripts/repair_confirmed_orphan_records.py`: an ID-locked, repository-target-locked utility that defaults to dry-run, requires `--apply`, refuses mismatches/unknown targets, verifies live/cloud/historical read-only boundaries, creates a hash-verified backup, repairs an isolated candidate, reopens and verifies it, and atomically promotes only to `data/CSPM.xlsm`. A repeated explicit apply returned `already_repaired` with `mutation: none`.
- [x] Used the established exact-key table-row deletion and candidate/promotion convention. Removed exactly two rows from `tblTimeEntries` and one row from `tblMatters`; all other table rows, row styles, financial controls, formulas, defined names, validations, workbook/package structure, and protected-invoice values compare unchanged. The macro-enabled content type and all 77 package members remain; the workbook had no VBA project binary before or after.
- [x] Preserved before-state evidence at `backups/CSPM/repository_record_repair_20260830_161006Z/CSPM.before-repair.xlsm`, candidate at `backups/CSPM/repository_record_repair_20260830_161006Z/candidate/CSPM.xlsm`, and sanitized audit/rollback instructions at `backups/CSPM/repository_record_repair_20260830_161006Z/repair_audit.json`. The verified backup SHA-256 equals the original repository SHA-256 `98FB1AC9176A959A11E1614CCB870BE5EF15327AEB453F74F2EAFA35E4A613B1`; candidate and promoted SHA-256 both equal `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`.
- [x] Canonical candidate and promoted checks each passed: **12 tables / 1,537 rows / 0 errors / 7 warnings**. The target `tblMatters.MatterNumber` error is gone; the seven independent warnings remain exactly four `missing_account_reference` and three `missing_business_unit_reference` warnings.
- [x] Sandbox-safe validation: Python compilation passed; the 12 required repair behaviors ran as **17 passed in 78.93s**; adjacent workbook-integrity, matter, time-entry, archive, and deletion tests ran as **17 passed in 9.14s**. Post-promotion disposable-copy reruns passed **3 tests in 19.88s** for exact scope/structure/repeat execution and **9 tests in 47.92s** for exact scope, all six financial-dependency refusals, protected-invoice preservation, and repeat execution after final utility hardening. No QML or PowerShell source changed, so QML lint and PowerShell parser checks were not applicable.
- [x] Recalculated all six governed fingerprints. Configured live/cloud CSPM and Dockets hashes and the historical-evidence hash are unchanged; only the repository snapshot changed. Conflict checking remains inactive. No commit, push, EXE build, live/cloud publication, warning repair, or startup hook was performed.
- [x] The separate Custom Fee production-behavior issue is implemented and validated in the dedicated section above; it did not alter this completed historical repair.

## Phase 5.5 Single Required-Value Integrity Error — Resolved by Confirmed Repository-Only Removal (2026-08-29)

- [x] Ran the canonical governed read-only check through `scripts/check_workbook_integrity.ps1`. It checked **12 tables / 1,540 rows** and reproduced exactly **1 error / 7 warnings** without changing the repository snapshot bytes.
- [x] Sanitized error: `missing_required_value` in `tblMatters.MatterNumber`, affecting one row in the divergent repository snapshot, stable `MatterID` `17bcaa4f-658d-48b0-84e7-01c4411ce135`.
- [x] Root-cause classification: genuinely missing authoritative data in one sparse legacy/out-of-band matter row. The row is absent from the August 6 committed workbook revision and first appears unchanged in the August 10 committed workbook snapshot. The schema and header mapping are correct, the required-field rule predates the row, and repository-supported matter-profile and typed-docket creation paths throughout available history generate a nonblank matter number. The row has linked time records, but no authoritative matter number, opening date, matter type, creation/update timestamp, matching legacy matter-registry record, or exact source-docket match from which the number can be reconstructed safely.
- [x] Added `tests/test_workbook_integrity_matter_number.py`. It proves the exact invalid condition is detected without exposing synthetic confidential names; a valid matter passes; both supported write paths cannot reproduce the blank number; the check is byte-for-byte read-only; ZIP/table structures survive; seven separate reference warnings remain visible; reload verification succeeds; and conflict schema/A08 remain inactive.
- [x] Sandbox-safe validation: focused new tests **4 passed in 8.15s**; new plus adjacent matter/repository tests **20 passed in 17.82s**; canonical authoritative checker exit **1** with the same **1 error / 7 warnings** and identical before/after workbook SHA-256; no QML or PowerShell source changed, so no QML lint or parser run was applicable.
- [x] Diagnosis-time data status: **not changed**. No checkout, backup, publication, template synchronization, warning repair, conflict implementation, commit, push, or EXE build was performed during diagnosis.
- [x] Superseded by Cory's 2026-08-30 factual confirmation: the row was an artificial sparse matter with exactly two abandoned duplicate time dependents. The three authorized rows were removed through the governed repository-only repair above; no `MatterNumber` was fabricated.
- [x] No interactive Qt/WebEngine validation applies to this workbook-only repair. Reopen, package, structural, control-total, and canonical integrity validation completed on both isolated candidate and promoted repository snapshot. Conflict checking remains inactive.
...
## Governed `launch.ps1` Null LiteralPath Blocker (2026-08-29)

- [x] Read the runtime log before diagnosis. It showed no fresh launch attempt, confirming the reported failure occurred before Python/WebEngine startup.
- [x] Identified the full failure chain. The initiating error was `scripts/ensure_venv.ps1:287`: its requirements-stamp code called `Get-FileHash`, but a Windows PowerShell child inheriting PowerShell 7's module search path could see an incompatible `Microsoft.PowerShell.Utility` first and could not resolve that cmdlet. The former script-wide `trap` in `launch.ps1` then called `Test-Path -LiteralPath $transcriptPath`; traps apply to the entire script scope, while `$transcriptPath` had not yet been assigned. That secondary null-path binding failure replaced the useful `Get-FileHash` error and was attributed opaquely to `launch.ps1`.
- [x] Replaced the host-dependent hash cmdlet with the equivalent .NET SHA-256 stream implementation, removed the redundant global trap, guarded empty resolver candidates, and made governed resolver errors preserve source file/line. The existing application `try/finally` continues to collect console/transcript output. No default path, alternate startup route, skipped initialization, workbook change, or payment change was introduced.
- [x] Sandbox-safe validation: PowerShell parsed `launch.ps1` and `scripts/ensure_venv.ps1` with **0 errors**; `launch.ps1 -SetupOnly` passed under both Windows PowerShell 5.1 and PowerShell 7 and resolved `C:\Projects\__CSPM\.venv_CORY_CorySchneider\Scripts\python.exe`; direct `ensure_venv.ps1 -InstallRequirements -PassThruPython` returned the same interpreter. The formerly failing detached Windows PowerShell host now exits **0** with empty stderr, and its .NET requirements digest exactly matches `Get-FileHash` (`5A95144BB041979750D3F05DC637F7241A1B8D8C8F47E4F6208610C3A5142306`). A clean-child null-path probe exits **1** with `Missing ensure script`, not a null-`LiteralPath` error. `git diff --check` passed. PSScriptAnalyzer is not installed.
- [x] Confirmed independent from the unpaid-invoice filter: that work changed payment QML/backend/tests only; it did not change `launch.ps1`, venv resolution, or startup configuration.
- [x] Outside-sandbox governed validation of the final edited `launch.ps1` reached the main window, `post-settle-ready`, and recorded real user input in a fresh `logs/cspm.log`; it then recorded a normal user-initiated close and clean shutdown. No alternate Python/application command was used. Reopen for the blocked manual acceptance with `& 'C:\Projects\__CSPM\launch.ps1'`.

## Make Payment Unpaid-Invoice Filtering (2026-08-29)

- [x] Reworked the shared `listOpenPaymentInvoices` path so Payment Entry searches its already eligible open Receivables rows by invoice number, work/client name, matter description, billing client, or the displayed open balance within an inclusive ±5% range.
- [x] Reused the canonical read-only invoice-to-matter context used by statements; no `tblReceivables` schema change, payment/allocation change, or workbook write was introduced. Paid, voided, reversed, superseded, closed, cancelled, and non-positive-balance rows remain excluded.
- [x] Made monetary query parsing Decimal-safe for `$`, commas, decimals, whitespace, and a typing-friendly trailing decimal point. The selector continues to use **Balance** (open `BalanceDue`), not `TotalInvoiced`, because Balance is the amount displayed to the user.
- [x] Added `tests/test_payment_invoice_filtering.py`; focused Payment Entry tests passed (**7 passed**) and adjacent invoice/payment/receivables/statement/financial-sync tests passed (**36 passed**). Python compilation passed. Governed QML lint reported existing unqualified-access warnings only, with no syntax error.
- [ ] Manual outside-sandbox acceptance: run `.\launch.ps1`, open Make Payment, and confirm a known open invoice can be found by its matter, billing client, invoice number, and displayed balance while a paid/void/reversed invoice never appears. Confirm the selected invoice and entered partial-payment amount remain stable while changing the filter.

## Invoice Courtesy Discount Workflow Fix & Invoice 26-0070 Recreation (2026-08-24)

- [x] Fixed courtesy discount persistence, type normalization, and auto-commit across `InvoiceBuilderWorkspace.qml`, `InvoiceBuilderView.qml`, `BillingController`, and `InvoiceDraftService`.
- [x] Added automated test coverage in `tests/test_invoice_discount.py` for both flat and percentage courtesy discounts, tax recalculation on net fees, and WIP ledger balancing in `finalize_draft`.
- [x] Reversed and recreated invoice `26-0070` for `Kingdon Green LP` dated July 6, 2026 with the `$4,327.00` flat discount ($8,023.00 net fees, $1,042.99 HST, $9,065.99 total due).
- [x] Exported Concept_A2 HTML and finalized PDF to `C:\Users\CorySchneider\OneDrive - LPN\__Invoices (1)\__RECEIVABLE\INV 26-0070 - Kingdon Green LP - 06JUL2026.pdf`.

## P0 Correction — Packaged Splash and Professional Window Motion (2026-08-20)

- [x] Read the fresh taskbar-executable log before changing the reported visual
  defects. It proves the painted splash intro ran from 18:05:00.189 to
  18:05:00.815, but the prior `OutCubic` front-loaded its opacity and read as a
  pop. It also proves Professional maximize/restore prepared the native host
  but never emitted `Professional one-owner texture motion started` or
  `Professional compositor motion settled`; the user therefore saw only the
  one-time native geometry jump.
- [x] Make the CS mark's painted intro an unmistakable 960 ms `InOutCubic`
  dissolve/rise and synchronously repaint each brief visible frame so DWM
  cannot coalesce it into the final opaque logo.
- [x] Start the Professional scene-graph timeline in the same QML transaction
  as its one-time host staging, rather than an unreliable deferred callback;
  anchor the source texture to actual native-window coordinates while Windows
  applies that staging.
- [x] Run sandbox-safe Python compilation, focused maximize/splash/startup
  regression tests (**18 passed**), and governed QML lint (existing
  warning-only diagnostics; no syntax error).
- [x] Build, hash-verify, and promote this correction to `dist\CSPM\CSPM.exe`.
  The verified source and installed trees both contain **4,358 files** and
  **678,579,033 bytes**, SHA-256
  `6E2C1686BC1463ACEB6C02764AF5077DE97E3D10D2D5B8A523D35019F7076B37`.
  Installed `CSPM.exe` SHA-256 is
  `8DAC3B53B3D208463A686D4ED38E6B93F0B2CAD3EFEBE5F1D3610BC0BEB282AC`.
  The prior package is recoverable at
  `to_delete\dist__manual_replaced_release_20260820_183040`.
- [ ] Manual P0 acceptance in the new packaged taskbar EXE: the CS mark must
  visibly dissolve/rise on launch, and each Professional maximize/restore must
  be one continuous transform without a pre-jump, native-resize sweep, blank
  frame, or layout reflow. Preserve the fresh package log if any defect stays.

## Professional Window Motion and Native-Splash Rebuild (2026-08-20)

- [x] Read the fresh taskbar-package runtime log before changing visual code.
  It confirmed the reported visible defects: the native splash recorded no
  authored intro start, and the reported restore attempts had no coherent
  Professional motion trace.
- [x] Replace the Professional maximize/restore implementation with one
  compositor-owned `contentLayer` texture motion. The native host stages once;
  a single 240 ms `OutCubic` progress value owns all intermediate pixels; the
  live responsive layout is not resized or reflowed until the final rectangle.
  Remove the obsolete Professional `grabToImage`/snapshot image/fallback path.
- [x] Replace Windows top-level splash opacity/position animation with a
  620 ms painted-content dissolve, 18 px rise, and subtle 96.5%→100% settle.
  The progress indicator follows that content intro and now catches up to
  authoritative readiness milestones at 1.20 units/second instead of 0.20.
- [x] Run sandbox-safe `py_compile`, focused maximize/splash/startup regression
  tests (**18 passed**), `git diff --check`, and the governed QML lint wrapper
  (existing warning-level diagnostics only; no syntax error).
- [x] Build, hash-verify, and promote this revision to
  `dist\CSPM\CSPM.exe` without replacing the user’s recoverable candidates.
  Candidate and installed manifests match: 4,358 files, 678,577,278 bytes,
  SHA-256 `4D7B91535F2173D21EFEE6534BD09844FF91AE6F919B6C71C6928EF97E9BB334`.
  `CSPM.exe` SHA-256 is
  `7E1C0F9730F01F79E2B4F827A0EBD5FFFCE05070FF5F0C2929A00E400BF54318`;
  the preceding package is recoverable at
  `to_delete\dist__manual_replaced_release_20260820_133443`.
- [x] Real outside-sandbox Qt/WebEngine startup check: the promoted EXE logged
  the new native content-intro start and completion, became responsive, and
  reached its first main pixel without QML/runtime error. Its cold first-main
  pixel was 53.688 s; the remaining material delay is a 34.955 s hidden-shell
  compilation, not the painted progress catch-up.
- [ ] Manual P0 acceptance in the packaged taskbar EXE: maximize and restore
  repeatedly in Professional style. Each must be one uninterrupted 240 ms
  surface transform with no pre-jump, native resize sweep, blank frame, or
  responsive layout reflow. Then confirm the CS logo truly dissolves/rises
  into place at launch and the loader reacts promptly to real milestones.

## Packaged Native Splash Intro and Cold-Start Responsiveness (2026-08-20)

- [x] Read the actual taskbar-package runtime log before changing the splash.
  The old August 18 EXE was serialized for **138.796 s** before first pixel;
  shell compilation and object construction were the dominant stalls.
- [x] Restore the authored native CS logo introduction. The transparent,
  painted first frame now runs a 460 ms opacity-and-rise animation before
  synchronous startup work can skip directly to the final opaque logo.
- [x] Compile the hidden `DetachedShellWindow` asynchronously so the native
  splash stays responsive and the isolated Practice Briefing worker begins in
  parallel with QML compilation. The progress bar remains driven only by real
  readiness states.
- [x] Run sandbox-safe Python/QML checks and focused startup regressions
  (**9 passed**). The mandatory QML lint wrapper reported existing warnings
  only and no syntax error.
- [x] Build and hash-verify the complete 2.4.0 taskbar package, then promote
  it to `dist\\CSPM\\CSPM.exe`. The 4,358 installed files match the candidate
  manifest `5456F07DA177E3B6120382A988031FFA65D354C10C39CD4D0ED462D92342608D`;
  the preceding package is recoverable at
  `to_delete\\dist__manual_replaced_release_20260820_114642`.
- [x] Real package startup check outside the sandbox: two launches completed
  without QML error/crash and reached first main pixel at **32.056 s** and
  **33.420 s**, respectively (down from the prior 138.796 s path). The
  residual cold cost is the approximately 22 s large-shell compile, now
  parallel with the authoritative briefing read.
- [ ] Manual acceptance: launch `dist\\CSPM\\CSPM.exe` from the taskbar.
  Confirm the CS logo visibly fades and rises into its final position rather
  than appearing fully opaque, the bar remains visually alive while real work
  proceeds, and the splash-to-plasma-to-bloom handoff stays clean. Do not
  alter another startup state machine until this path is confirmed in the
  real app.
...
- [x] Build and hash-verify the complete 4,361-file candidate package, promote it to the canonical `dist/CSPM` location, deploy the identical executable to `C:\Programs\CSPM`, and retain the preceding release in `to_delete`.
...
- [x] Correct the Billing Client A/R Collection work-client filter row so its floating **Client** label and **All clients** value use the standard labelled-field height instead of clipping inside a 36 px control; rebuild and redeploy the verified package.
- [x] Add click-to-allocate behavior to Billing Client A/R Collection invoice rows: a row click fills **Apply ($)** with that invoice's full outstanding balance, a second click clears it, allocated rows receive clear visual feedback, and the amount remains directly editable without changing **Amount received ($)**.
- [x] Build, deploy, and real-WebEngine smoke-test the click-to-allocate release. Dist and installed trees match at 4,275 files / 674,269,738 bytes / manifest SHA-256 `9B9A633BDF4682D2291997BEBFCC346E84443F0C12BEA0D2248015AFAF83A73D`; the installed executable reached input-ready and closed normally.
