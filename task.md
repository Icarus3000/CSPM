...
- [x] Add a live collection-list filter across invoice number, work client, matter, and invoice date. The visible-row count and shown A/R total update with the filter, and **Allocate shown oldest first** intentionally allocates only the filtered invoices.
...
- [x] Sandbox-safe validation: Python compilation passed; focused and adjacent payment/invoice/report tests passed (**63 passed**); both changed QML files compiled `Ready`; governed lint reported no errors (the pre-existing Payment Entry warning diagnostics remain); `git diff --check` passed.
- [x] Rebuilt the complete candidate-validation executable at `dist\CSPM\CSPM.exe` and deployed the same package to `C:\Programs\CSPM\CSPM.exe` (matching SHA-256 `0F8B152518689ECD229CD0DFC26870B411271E9C7B2FCBF7930F8C441A3EF8D6`; 4,325 build files). Installed collection-dialog QML matches source byte-for-byte. The preceding dist package is recoverable at `to_delete\dist__replaced_release_20260910_104341`.
- [x] Outside-sandbox Qt/WebEngine startup validation passed from the updated installed path. The app reached first main-window pixel in 15.140 seconds, remained responsive, and closed through the normal transition. No payment was posted; shutdown released the shared checkout marker.
- [ ] Manual application acceptance: open **Payment & Adjustment Entry** → **Collect billing-client A/R**; filter a multi-invoice billing client with `Gold`; confirm only matching rows remain; type a multi-digit/decimal amount continuously in Apply and confirm focus/scroll position remain stable. Then, against a recoverable test workbook, balance and post a receipt and verify one transaction plus matching per-invoice ledger/payment-history entries.
