...
- The collection list now has a live invoice/client/matter/date filter, so a term such as `Gold` narrows the visible remittance candidates and recalculates the shown count/A/R total. When filtered, **Allocate shown oldest first** clears hidden allocations and applies the receipt only to visible invoices, avoiding an accidental off-screen allocation.
- The original Apply editor replaced the complete JavaScript array on every `onTextEdited` event. QML consequently destroyed/recreated the ListView delegate after each character, dropping focus and shifting the viewport. `setAllocation()` now mutates the stable row object and increments a lightweight totals revision; the active delegate is made current and retained with `ListView.Contain`, while full model replacement is reserved for deliberate filter/auto-allocation/reset operations.
...
- Added focused repository coverage for one parent transaction/two allocations, A/R/WIP synchronization, pre-write total and party validation, read-only allocated-receipt history, plus QML/controller contract coverage. Sandbox-safe validation passed: Python compilation; **63 focused and adjacent tests**; both changed QML components compiled `Ready`; governed QML lint had no errors (existing `PaymentEntryView.qml` warnings remain); and `git diff --check` passed.
- The corrected candidate-validation build produced a complete 4,325-file package at `dist\CSPM`; `CSPM.exe` SHA-256 is `0F8B152518689ECD229CD0DFC26870B411271E9C7B2FCBF7930F8C441A3EF8D6`. It deployed automatically through the repository's user-data-preserving deployment function to `C:\Programs\CSPM`; installed/dist executable hashes match, and source/dist/installed `BillingClientARCollectionDialog.qml` hashes match at `FD60FA59DE8E5CAB1BFECDE6217CB8DBF69D349088C79C4E5075FFC606B2942C`. The preceding dist package is recoverable at `to_delete\dist__replaced_release_20260910_104341`. A production-approved build remains intentionally unavailable while the template is `CANDIDATE_AWAITING_CORY_APPROVAL`.
- Outside-sandbox Qt/WebEngine validation of the updated installed executable reached first main-window pixel at 15.140 seconds, remained responsive, and closed through the normal application transition; no shared checkout marker remains. Foreground acceptance is to filter with `Gold`, type a multi-digit/decimal allocation without focus or viewport loss, and then test posting only against a recoverable workbook. No financial data was changed during automated validation.
