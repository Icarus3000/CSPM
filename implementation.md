...
## 2026-09-24: Invoice 26-0109 Posting Recovery And Finalization Guard

- **Deployment:** source commit `1065a81` pushed to `origin/wip/ap-defect-validation`. The governed candidate-validation build deployed to `C:\Programs\CSPM\CSPM.exe`; installed/dist executable hashes match `723BA642AE9C449579A81E4BBF77876A1EF4E65ADDBEF65B30454105CC3F7B2A`, and installed Invoice Builder QML matches source. The previous package remains recoverable at `to_delete/dist__replaced_release_20260924_152510`. Outside-sandbox installed Qt/WebEngine startup reached input-ready at 15.200 seconds and closed normally; shutdown confirmed unchanged local/cloud data and released the checkout. Interactive finalization acceptance remains manual.

- **Root cause:** Invoice Builder created additive flat-fee line `CF_be74fe7637655147b867` under draft `TRUEXPERIE-20260924-00F2-D`. Recreating the draft moved its ordinary `InvoiceRef` to `TRUEXPERIE-20260924-CCE7-D` and deleted the orphaned old draft, but left `DraftOwnerID` and `DraftRef` in `LockAudit` pointing to `00F2`. The finalization invariant correctly rejected the split ownership. Because QML exported the PDF before invoking the financial commit, the user received a final-looking `26-0109` document even though no Invoice Log, Receivable, or Ledger record existed.
- **Lifecycle repair:** `InvoiceDraftService.create_draft()` now rebuilds the structured audit markers whenever a Draft-state custom fee moves to a replacement draft. It preserves the validated request ID, stable line ID, Draft state, and normalized fee treatment while atomically changing the stable owner and draft reference. The new lifecycle regression recreates a draft containing an additive custom fee, verifies every marker, and successfully finalizes the replacement.
- **Truthful finalization sequence:** Invoice Builder still prepares the finalized HTML before posting, but now commits the workbook first and exports the final PDF only after `draftFinalized(ok=true)`. A posting failure therefore produces no final-looking file. A subsequent PDF write failure reports the file error without rolling back or visually denying the already-authoritative accounting post. Removing the nonexistent `onDraftFinalizationError` handler also eliminates its runtime signal warning.
- **Disbursement WIP authority:** supplier `PaymentStatus` describes whether the supplier expense is pending/paid; it is not client invoice status. WIP loading now includes Invoice Log and Receivables in the same immutable bulk snapshot and excludes a disbursement whose `InvoiceRef` is posted there. Regression coverage proves that a posted/PENDING disbursement is absent while a PAID but unbilled expense remains billable.
- **Live repair:** after governed checkout and timestamped backups, the stale custom-fee owner was rebound to draft `CCE7` and the normal finalization service posted `26-0109`. The verified result is one Invoice Log, one Receivable, and one Revenue ledger row: `$1,250.00` professional fees + `$491.06` tax-exempt disbursement + `$162.50` HST = `$1,903.56` total/open. The draft is gone, three substantive time/fee rows plus the internal `$270.00` discount adjustment link to the invoice, and `DISB-20576109-PATCH` is linked once.
- **Returned source docket:** the original one-hour `$950.00` docket had been deliberately removed from the draft before the additive flat-fee line was created. It was preserved as auditable time history and marked `Reconciled` with `SEE FLAT FEE 26-0109`, zero invoice balances, and an explicit custom-fee replacement note. It is not an invoice monetary row and cannot be rebilled. The corrected source-code WIP projection returns zero Truexperiences entries.
- **Data protection:** pre-post backup: `backups/CSPM/manual_invoice_repairs/20260924-151833-invoice-26-0109`; pre-reconciliation backup: `backups/CSPM/manual_invoice_repairs/20260924-152023-invoice-26-0109-source-docket`. The governed cloud release is `20260924T192026Z_8c6f1469_bf48dd2c`; local/cloud `CSPM.xlsm` hashes match at `F1632E49E76E174ACA15CD67BAAC1D527BDB182CFEB03C04B9F14D1A0854FFA8`.
- **Validation:** sandbox-safe Python compilation passed; all **53** `test_invoice*.py` plus WIP workbench tests passed; governed QML lint completed with existing warning-only diagnostics and no syntax error; `git diff --check` passed with line-ending notices only. Installed Qt/WebEngine validation and the final candidate deployment are recorded separately after packaging.

## 2026-09-24: Invoice Builder Summary, Zen Layout, And Disbursement Labels

- The Invoice Builder now exposes the existing durable `ShowTotalHours` draft field as a **Show total professional time** checkbox. The setting is saved through `updateDraftMeta`, reloads the workspace/preview, and controls the summary for hourly-only invoices. The established mixed/flat-fee rule remains authoritative: total time is hidden whenever any flat fee is present even if the stored checkbox preference is on.
- `InvoiceBuilderWorkspace.qml` now has an explicit `focusMode`. In normal mode the draft/line-item pane uses a responsive width and the preview pane has a zero minimum width. Settings and action controls use wrapping flows with explicit minimum-width release, preventing their combined implicit widths from pushing the bottom-right controls outside a smaller or scaled monitor.
- Zen Preview now embeds that same workspace in `focusMode`, so it shows the live HTML preview together with invoice date, grouping, discount, agency split, reconciliation, time-docket display, total-time choice, flat-fee actions, and finalization controls. The draft list and editable line-item pane are omitted; **Exit Zen** returns to the complete builder.
- Invoice payload generation now resolves the supplier/payee through each disbursement's governed `SourceTransactionID`. Generic text such as `supplier invoice 20576109` becomes `CIPO invoice 20576109`. Tax-exempt disbursements receive a single `(Tax Exempt)` suffix before terminal punctuation, producing `CIPO Filing Fees - CIPO invoice 20576109 (Tax Exempt).` in the reported example.
- Sandbox-safe checks passed: Python compilation; **42 focused and adjacent tests** across mixed invoices, summary preference persistence, responsive/Zen contracts, custom-fee lifecycle, discounts, Invoice Directory, and payment filtering; governed QML lint exited successfully with the files' existing warning-only diagnostics; and `git diff --check` passed with line-ending notices only.
- PDF layout validation regenerated the mixed invoice through the real Jinja template, printed it with headless Microsoft Edge, rendered both pages with PyMuPDF, and visually inspected them. The supplier name and tax-exempt marker fit cleanly in the separate disbursements table; the summary remains aligned and omits professional time for the mixed flat-fee example.
- Release: source commit `c5e1fd7` was pushed to `origin/wip/ap-defect-validation`. The governed candidate-validation builder promoted and deployed a 4,361-file package to both `dist\CSPM` and `C:\Programs\CSPM`; EXE SHA-256 values match at `6926332C350F8D50C1423C9FAD6B143FED6F8463D76A21DA39B0492F59B5E4E6`, and packaged/installed `InvoiceBuilderView.qml` and `InvoiceBuilderWorkspace.qml` match source exactly. The previous dist package is recoverable at `to_delete\dist__replaced_release_20260924_145913`.
- Outside-sandbox installed Qt/WebEngine validation reached first main-window pixel and input-ready, then closed through the normal Professional transition with exit code 0. Shutdown reported cloud authority and the local workbook replica already matched and released the checkout; verified local/cloud `CSPM.xlsm` and `Dockets.xlsm` hashes remain identical. No live draft or finalized invoice was changed during automated checks. Foreground acceptance of Zen and the wrapped monitor layout remains pending on the user's reported display.

## 2026-09-24: Mixed Hourly / Flat-Fee / Disbursement Invoices

- Split the previous overloaded fee concept into two explicit treatments. **Set Invoice Flat Fee** remains the historical invoice-wide replacement mode: it supersedes ordinary professional WIP and can expose the difference as a courtesy discount or preserve it as a hidden reconciliation adjustment. **Add Flat Fee Line** is a new additive amount-only professional-services line with no hours or rate; it coexists with hourly work and does not negate it. Historic draft-owned custom-fee records remain invoice-wide for compatibility, while ordinary fee entries and legacy amount-only rows are treated as additive.
- `InvoiceDraftService` now carries `FeeTreatment` through creation, audit metadata, recalculation, and finalization. Only invoice-wide replacement fees trigger WIP replacement/reconciliation. Multiple deliberate additive lines are allowed while the existing idempotency and draft-ownership protections remain in force; an additive line cannot be mixed with an already-selected invoice-wide replacement fee.
- `BillingController` now builds separate hourly, additive-flat-fee, invoice-wide-flat-fee, and disbursement collections for the invoice renderer. Discounts and agency deductions apply only to professional fees. HST is calculated on net professional fees plus taxable disbursements, total due includes disbursements, and final Invoice Log rows record professional fees separately from `TotalDisbursements` while the ledger retains the combined pre-tax billing value.
- `InvoiceBuilderView.qml` and `InvoiceBuilderWorkspace.qml` expose clearly named **Add Flat Fee Line** and **Set Invoice Flat Fee** actions with explanatory dialog text. The invoice template renders **Professional Services**, **Flat Fees**, and **Disbursements / Client Expenses** independently; it omits total professional time whenever any flat fee is present and lists disbursements separately in the summary.
- Added regression coverage for the user's exact example: `$190.00 + $380.00` hourly services, a `$950.00` additive flat fee, a `$491.06` tax-exempt disbursement, and a `$270.00` professional-fee discount produce `$1,250.00` net professional fees, `$162.50` HST, and `$1,903.56` total due. Finalization records `$1,250.00` professional fees and `$491.06` disbursements separately, with `$1,741.06` combined pre-tax ledger billings.
- Sandbox-safe validation passed: Python compilation; **58 focused and adjacent tests** covering fee lifecycle, mixed rendering/finalization, discounts, LIHDC, reversals, Invoice Builder, Invoice Directory, payments, and statements; governed QML lint completed with warning-only existing diagnostics; and `git diff --check` passed. A broader keyword-selected run encountered three unrelated collection failures from existing import-path and stale motion-settings-test issues.
- PDF layout validation used the real HTML template through headless Microsoft Edge, then rendered both PDF pages to images with PyMuPDF for visual inspection. The first pass exposed matter-header clipping under the absolute page-one header; increasing the first-page clearance fixed it. The final render cleanly separates hourly services, flat fees, and disbursements and shows the corrected summary without a total-hours line.
- Release: commit `9f7b88c` was pushed to `origin/wip/ap-defect-validation`. The production build gate correctly refused the still-unapproved workbook template, so the governed `--candidate-validation --validate` path built and deployed the non-production validation package without changing approval status. Dist and installed package trees each contain 4,361 files; `CSPM.exe` hashes match at `EB31CF24D8E489BDC1B1884893169BA39EB104082F1A47B09D035A12DB365EF0`, and the packaged/installed invoice-builder QML and `Concept_A2.html` hashes match source exactly. The previous dist package is recoverable at `to_delete\dist__replaced_release_20260924_101903`.
- Outside-sandbox Qt/WebEngine validation launched `C:\Programs\CSPM\CSPM.exe`, reached first main-window pixel at 9.248 seconds and input-ready at 10.259 seconds, then closed through the normal Professional transition with exit code 0. The governed service recovered the dead validation lease left by an earlier mis-scoped log probe, recorded its audit under LocalAppData, acquired a fresh checkout, published an unchanged package, and released the checkout. Local/cloud `CSPM.xlsm` and `Dockets.xlsm` hashes match exactly and no marker remains. No invoice or other financial record was written; foreground mixed-fee acceptance on a disposable draft remains pending.
...
- Sandbox-safe validation: `python -m py_compile src/python/services/invoice_draft_service.py` passed; **55 tests passed** across custom-fee lifecycle, discounts, reversal, LIHDC sharing, Invoice Builder responsiveness, Invoice Directory, invoice filtering, and payment entry; `git diff --check` passed with line-ending notices only.
- Release: commit `bf12768` was pushed to `origin/main`. The governed candidate-validation builder completed and deployed the new package to `C:\Programs\CSPM`; dist and installed `CSPM.exe` hashes match at `C2DA86AE33A938FC796EF46FEC97A7DE6B0F7305B4ADBB792BB90E2FF865AA16`, and both package trees contain 4,274 files. The previous release is recoverable at `to_delete\dist__replaced_release_20260910_144425`.
- Outside-sandbox WebEngine validation: the installed executable reached `ready-to-reveal`, rendered first main-window pixel at 12.451 seconds, remained responsive, and closed normally through the application transition. Shutdown reported that cloud authority and the local workbook replica already matched, then released `.cspm_checkout.json`. No invoice, payment, or other financial record was written during the smoke test. Interactive custom-fee financial acceptance remains pending on a new disposable draft; `26-0097` remains excluded.
...
- The dialog loads all open A/R for one exact billing client and captures date, total received, method, general deposit account, external reference, and notes once. Each invoice has an editable allocation and projected balance. **Allocate oldest first**, clear, total A/R, received, allocated, and unallocated controls support fast review; posting is blocked unless positive allocations exactly equal the received amount and no allocation exceeds its invoice balance.
- The initially added combined invoice/client/matter/date text filter was misleading in a billing-client-scoped collection session. It is now a dedicated searchable **Client** selector populated only with distinct work clients represented by that billing client's open invoices, with **All clients** first. Selecting `Gold` performs an exact case-insensitive work-client match, leaves the payer/billing client unchanged, and recalculates the shown count/A/R total. **Allocate client oldest first** clears hidden allocations and applies the receipt only to the selected client's invoices, avoiding an accidental off-screen allocation.
- The original Apply editor replaced the complete JavaScript array on every `onTextEdited` event. QML consequently destroyed/recreated the ListView delegate after each character, dropping focus and shifting the viewport. `setAllocation()` now mutates the stable row object and increments a lightweight totals revision; the active delegate is made current and retained with `ListView.Contain`, while full model replacement is reserved for deliberate payer changes, auto-allocation, and reset operations.
- `ExcelRepo.post_billing_client_receipt()` validates the entire command before writing. It creates one cleared general-account `Transactions Master` row for the actual receipt and embeds structured allocation evidence in `ExpenseDetails`. It then creates one `Ledger` row per invoice, all sharing the receipt transaction ID, and synchronizes Receivables plus linked Time and Disbursement payment fields inside one `with_financial_write_batch` commit.
- Multi-invoice allocations appear as `Allocated receipt` in invoice payment history. They are deliberately non-editable through the existing single-invoice Payment Entry editor, which now also rejects a direct attempt, because changing one child allocation without governing the parent receipt would break bank reconciliation. A whole-receipt correction/reversal path remains a future enhancement.
- The current workflow is explicitly general-account-only. It does not accept trust money or model a trust transfer. The regulated roadmap now records the integration boundary: a future authorized trust-to-general transfer may become a source receipt only when it is linked to the trust event, client/matter liabilities, rendered invoices, Form 9A evidence where applicable, and trust reconciliation controls.
- Added focused repository coverage for one parent transaction/two allocations, A/R/WIP synchronization, pre-write total and party validation, read-only allocated-receipt history, plus QML/controller contracts for the exact work-client selector and stable Apply editor. Latest sandbox-safe validation passed **37 related payment/invoice tests**; the revised dialog compiled `Ready`; governed QML lint had no errors; an offscreen QML smoke exercised case-insensitive exact client filtering; and `git diff --check` passed with line-ending notices only.
- The latest candidate-validation build produced a complete 4,325-file package at `dist\CSPM`; `CSPM.exe` SHA-256 is `4ED700C3F772F39FD75CED69388E64206350992066A89AFA8830F7327A805F31`. It deployed automatically through the repository's user-data-preserving deployment function to `C:\Programs\CSPM`; installed/dist executable hashes match, and source/dist/installed `BillingClientARCollectionDialog.qml` hashes match at `3067680921323FFFB099327A61CBF96CE0E63D5F7F5C4CF0CC7AB5D8CFF9E314`. The preceding dist package is recoverable at `to_delete\dist__replaced_release_20260910_110311`. A production-approved build remains intentionally unavailable while the template is `CANDIDATE_AWAITING_CORY_APPROVAL`.
- Outside-sandbox Qt/WebEngine validation of the updated installed executable reached first main-window pixel at 14.240 seconds, remained responsive, and closed through the normal application transition; shutdown published an unchanged cloud package and no shared checkout marker remains. Foreground acceptance is to select `Gold` in the exact **Client** selector, type a multi-digit/decimal allocation without focus or viewport loss, and then test posting only against a recoverable workbook. No financial data was changed during automated validation.

## 2026-09-10: Persistent User-Defined Matter Types

- The New Matter Wizard previously owned a static QML `practiceAreaMatterTypes` map. General exposed only `General Consultation`, `Notary / Commissioning`, and `Other`, and there was no API or durable storage for a user-created value. The Matter Type control also initialized before Practice Area during new/edit hydration, allowing the model refresh to clear or mismatch a saved value.
- `AppController` now owns an expanded `DEFAULT_MATTER_TYPES_BY_PRACTICE_AREA` catalog and exposes `getMatterTypeOptions(practiceArea)` plus `addMatterTypeOption(practiceArea, matterType)`. Labels are single-line and whitespace-normalized, limited to 80 characters, and deduplicated case-insensitively. `Other` is guaranteed for every practice area.
- User-created entries are stored by practice area in LocalAppData `user_settings.json` under `customMatterTypesByPracticeArea`. The normal atomic settings writer preserves them across restart and package deployment. The returned catalog additionally merges distinct Matter Type values already observed in `list_matter_directory()`, grouped by each row's Practice Area, so historical values remain available even when they predate the catalog.
- `PlaceholderSubmenuView.qml` now refreshes the catalog when Practice Area, backend readiness, active wizard state, or the new backend signal changes. A compact `+ Add` button opens a modal that explains persistence, validates errors inline, saves the label, immediately selects it, marks the draft dirty, and gives status feedback. New and edit hydration now applies Practice Area, refreshes options, and only then applies the default/saved Matter Type; editing still preserves an unknown legacy value.
- Added `tests/test_matter_type_options.py` covering catalog breadth, whitespace normalization, settings persistence/reload behavior, duplicate prevention, workbook-observed values, write-failure rollback, and the QML workflow contract. Sandbox-safe checks passed: Python compilation, **5 focused tests**, governed QML lint with no syntax errors and the large host's existing warnings only, and `git diff --check`. The wider suite reached **205 passed / 6 unrelated existing failures** in orphan-repair/workbook-integrity and stale startup-contract tests; `test_motion_settings_suite.py` separately remains uncollectable because it imports removed constants.
- A full candidate-validation build was required because the governed CSPM workbook template remains `CANDIDATE_AWAITING_CORY_APPROVAL`. The complete 4,273-file / 674,123,706-byte package was promoted to `dist\CSPM` and deployed to `C:\Programs\CSPM`; both executables have SHA-256 `06EB350DA4DC9494204824BF39505CE80E6653459B96CD2F0F2A5EAC64762090`, and source/dist/installed `PlaceholderSubmenuView.qml` hashes match. The replaced package remains at `to_delete\dist__replaced_release_20260910_075928`.
- Outside-sandbox real packaged Qt/WebEngine validation reached first main-window pixel in 12.339 seconds with a writable shared-data checkout. The agent-started app closed through the normal Professional transition, published an unchanged cloud package, and left no checkout marker. The exact `+ Add` interaction and restart persistence remain for Cory's foreground acceptance; no test Matter Type or workbook record was created automatically.

## 2026-09-10: Metadata-Only Cloud Conflict And Legacy Import Write Preflight

- The fresh installed-app log showed startup returning `Both the local replica and cloud authority changed`, opening CSPM read-only, and the Legacy Dockets import later failing in `ExcelRepo._flush_import_batch()` when `SyncService.assert_write_lease()` rejected the final save. That late guard explained why the modal displayed 1,305 / 1,684 processed rows before stopping.
- Read-only ZIP-package comparison proved this was not competing practice-data work. Local and cloud `CSPM.xlsm` had 80 OOXML entries; 79 matched exactly, and the only difference was `docProps/core.xml` with a one-second `dcterms:modified` difference (`23:05:06Z` versus `23:05:07Z`). Worksheet XML, relationships, workbook structure, and the governed VBA payload all matched.
- `SyncService._workbook_payload_equal()` now hashes every non-directory OOXML entry after normalizing only the modified-at element. When that narrow proof succeeds, synchronization copies the canonical cloud file over the metadata-only local variant through `_copy_verified()`, retains the former local file under `sync_recovery`, and records the cloud file's complete SHA-256 as the common ancestor. Missing/duplicate/bad ZIP entries, any other core-property difference, or any substantive package-entry difference still returns false and follows the existing no-overwrite conflict path.
- Both `startLegacyDocketsImport()` and `startLegacyDocketsFilteredImport()` now assert the write lease synchronously before creating worker state. `legacyDocketsImportWritePreflight()` exposes the same check to `LegacyDocketsImportView.qml`, which performs it before resetting state or opening the progress popup. This prevents an avoidable multi-minute read-only import while preserving the repository's final save guard.
- The actual local/cloud pair was reconciled through the governed service. The former local file is recoverable at `C:\Users\cschn\AppData\Local\CSPM\data\sync_recovery\CSPM_before_metadata_alignment_20260910_071639_8fe40e7b.xlsm`. Current local/cloud hashes match exactly: `CSPM.xlsm` `A18A09AA22AA0984B637E147545B7790A93200008C57A6C6F1093894300FD2A3`; `Dockets.xlsm` `350CCB492C8030635983EEE96FFDBFA40788236C127E82EDA9AD74C6E24E4D8A`. No shared checkout marker remains.
- The first rebuilt installed-package validation exposed an independent deployment residue: the deployer deleted files no longer present in the new package but retained empty directories. An empty stale `_internal/numpy` directory was therefore importable as a namespace package, making OpenPyXL's optional dependency probe believe NumPy existed before failing on `numpy.short`. `deploy_to_programs()` now processes stale paths deepest-first and removes only empty directories absent from the source package; protected `data`, `backups`, and `logs` remain untouched. A regression test proves stale namespace directories disappear while current runtime files and user data remain.
- Sandbox-safe validation passed: `py_compile` for `sync_service.py`, `app_controller.py`, and `build_release.py`; **20 passed** across cloud checkout, metadata tolerance/strict payload conflict, Legacy Dockets write preflight, safe selection, reconciliation, and deployment cleanup; governed QML lint completed with existing warning-only diagnostics; `git diff --check` reported only line-ending notices.
- A complete candidate-validation package was built because the governed `CSPM.xlsm` template remains `CANDIDATE_AWAITING_CORY_APPROVAL`, promoted to `dist/CSPM`, and deployed to `C:\Programs\CSPM`. Dist and installed executable SHA-256 values match at `502B6F44CFBCB9D71273EE6F5523F1209A455FB2C1A4082C3B7100B16EED765B`; the executable has no `Zone.Identifier`, and the stale NumPy directory is absent. The replaced dist package is recoverable at `to_delete/dist__replaced_release_20260910_071450`.
- Outside-sandbox real Qt/WebEngine startup validation passed after cleanup: the installed EXE logged a successful writable checkout (`Cloud authority and local replica already match`) and reached first main-window pixel. The agent-started validation process was stopped; its checkout was recovered with immutable audit `C:\Users\cschn\AppData\Local\CSPM\checkout_recovery\20260910T112228Z_abandoned_checkout_934f45952a1a4907b4391db7a744e408.json`, then a fresh lease was cleanly released. Manual acceptance remains to repeat the reviewed six-row `88 queen` import; no import was run automatically.

## 2026-09-09: Same-Computer Cross-Launch Checkout Recovery

- The fresh installed-app log showed the exact cause of the Legacy Dockets Import failure. The packaged September 9 process (`PID 21068`) opened read-only because the shared `.cspm_checkout.json` marker still named dead source/runtime process `PID 15372` from September 8. The import correctly reached the single batch flush and then failed in `SyncService.assert_write_lease`; shutdown was blocked by the same guard.
- Automatic abandoned-checkout recovery did not run because `machineId` was stored under the launch-specific persistent runtime root. A source/dev launch and the packaged application could therefore present different installation IDs on the same physical computer even though they protect the same shared workbook package.
- `SyncService` now stores its default workstation identity at the stable LocalAppData CSPM path (with an explicit override available for isolated environments) instead of deriving it from a source/package runtime root. Source and packaged launches therefore use the same `machineId`. Recovery still requires that matching identity, the exact computer name, and Windows conclusively reporting the recorded PID as absent; it refuses unreadable markers, other installations/computers, live/unknown processes, and age-based recovery. The recovery guard and immutable audit-copy behavior are unchanged.
- The actual stale marker was recovered through `SyncService.checkout_from_cloud()`, with audit `C:\Users\CorySchneider\AppData\Local\CSPM\checkout_recovery\20260909T203559Z_abandoned_checkout_59418f1a23bf4dfa84e8f3bd00d81901.json`. A fresh checkout was acquired and cleanly released. No Legacy Dockets import or workbook mutation was performed; local/cloud hashes remained identical (`CSPM.xlsm` `A18A09AA...00FD2A3`, `Dockets.xlsm` `350CCB49...E4D8A`).
- Sandbox-safe validation passed: `py_compile` for `sync_service.py`; **14 passed** across cloud-checkout, Legacy Dockets safe-selection, and reconciliation tests; scoped `git diff --check` reported only the repository's line-ending notices.
- Outside-sandbox validation launched the real packaged Qt/WebEngine application. It logged a successful writable checkout (`Cloud authority and local replica already match`) and reached first pixel. The agent-started process was stopped after the check; its dead test lease was recovered through the same governed path and cleanly released, so no shared marker remains.
- A clean production build from committed `8024ccf` plus only the checkout repair was blocked by the intended template-governance preflight because `CSPM.xlsm` remains `CANDIDATE_AWAITING_CORY_APPROVAL`. No release was promoted and no unrelated dirty QML/window work was bundled. This does not block the immediate correction: the installed executable already uses the LocalAppData identity, the stale marker is gone, and the repaired source path now uses that same identity. Manual acceptance is to relaunch and repeat the reviewed six-row import.
...
- Recovery found that the interrupted work had already copy-promoted a `7fda5e9` fail-fast-only package to `dist`, preserving the failed `dce3112` package. The later clean detached worktree was at `d4196fcfb3d6af0b2ae396ff8d5fa474a18c16da`. No relevant PyInstaller, Python, CSPM, QtWebEngine, or build-wrapper process remained, so nothing was terminated.
...
- The existing schema is sufficient; no migration was needed. A draft's stable `DraftID` is the owner, a UUID5-derived `tblTimeEntries.EntryID` is the stable line identity, and a caller-generated request ID is the idempotency key. Structured `LockAudit` markers record `EntryType:Fee`, `FeeOrigin:InvoiceDraft`, `DraftOwnerID`, `DraftRef`, `RequestID`, `CustomFeeLineID`, `CustomFeeState`, and `FinalInvoice`. The row always carries a real matter reference belonging to the draft work client and already represented by that draft's time/disbursement work.
- `InvoiceDraftService` now serializes add/edit/remove/delete/finalize custom-fee lifecycle boundaries with a process-wide re-entrant lock. Add performs a coherent snapshot read, validates date/description/positive finite amount/matter ownership, writes Time + Draft totals in one bulk workbook replacement, and read-back recovers a successful but uncertain write. Same request ID returns the same line; different IDs permit deliberately identical fee content.
- Custom-fee edit preserves all ownership/identity markers and blocks when billed/payment/reissue/financial references exist. Remove destroys only the exact exclusively draft-owned custom-fee row. Ordinary dockets cannot be hard-deleted through this path and retain their established return-to-unbilled-WIP behavior. Draft deletion validates every owned custom fee before one bulk mutation, destroys those fee rows, releases ordinary time/disbursements, and removes the draft.
- Finalization validates each custom-fee owner/request/line/matter/dependency before mutating the coherent financial snapshot, marks the fee `Finalized`, and commits Time, Disbursements, Receivables, Invoice Log, Ledger, and Drafts once. A retry after a lost response is accepted only when the draft is absent, all linked custom fees consistently point to the requested billed invoice, and exactly one matching Receivable, Invoice Log, and Revenue ledger row exists. Incomplete, duplicate, mismatched, or billed-and-draft split footprints stop with an actionable diagnostics message.
- `BillingController` now exposes `newCustomFeeRequestId`, asynchronous `addDraftCustomFee`, a completion signal, and an in-flight request set. It continues routing any legacy fee submission into the authoritative idempotent command. Draft workspaces provide matter choices from both time and disbursement rows. `InvoiceBuilderView.qml` uses the shared Console/Professional dialog with a real matter selector, pending/Enter guards, per-opening request identity, same-request error retry, and no literal/fake custom-fee MatterID.
- Added `tests/test_invoice_custom_fee_lifecycle.py` with 14 focused lifecycle/contract tests. Combined sandbox-safe validation passed **32 tests in 11.25s** across the new suite plus invoice reversal, discounts, joint retainers, responsiveness, and LIHDC sharing. Python compilation passed. Governed QML lint completed without syntax failure; existing warning-level diagnostics remain. `git diff --check` passed.
- The canonical workbook was read-only throughout and remains SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`; canonical integrity remains **12 tables / 1,537 rows / 0 errors / 7 unrelated warnings**. Conflict checking was not started and no warning repair, EXE build, commit, push, or workbook change occurred.
- Cory completed a scoped outside-sandbox source-launch acceptance with `.\launch.ps1`: Invoice Builder opened on a safe real client/matter draft, a test custom fee was activated rapidly more than once, and exactly one line appeared with the intended behavior. This does not establish manual acceptance for edit/reopen, removal, abandonment, financial-dependency blocking, finalization/retry, or both Invoice Builder shells; those remain explicitly untested interactive scenarios.
- Release checkpoint: source commit `dce31122d44ca3455a292382f607affd96cfa20c` (`Harden invoice draft custom fee workflow`) was pushed to `origin/main` on 2026-08-30. A clean detached worktree at that exact commit passed the 14 Custom Fee lifecycle tests and Python compilation. Its selected adjacent suite result was 37 passed / 1 failed in 10.91s; the sole failure is the pre-existing LIHDC total assertion (`316.00` versus expected `221.20`), outside this Custom Fee checkpoint and not changed or repaired here.
- The governed QML lint wrapper completed against the exact committed Invoice Builder files with warning-level diagnostics only and no syntax failure. The committed workbook copy remained SHA-256 `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`, with 12 tables / 1,537 rows / 0 errors / 7 unchanged unrelated warnings.
- The canonical command `C:\Projects\__CSPM\.venv_CORY_CorySchneider\Scripts\python.exe scripts\build_release.py --validate` completed in the clean commit worktree. The hash-verified package manifest is 4,358 files / 678,615,222 bytes / SHA-256 `0EC666DA28A8134FC2299D44B149AD2CDF14ACB13C2F426861232B5DDF80CD24`; `CSPM.exe` SHA-256 is `9C7379024912E154E39F42528D58E1CDEC043E87A595D66EBBC5D36DEAFB36A6`.
- The standard `promote_verified_release_package.py` flow promoted the verified candidate to `dist` with an identical post-promotion manifest and preserved the prior package at `to_delete/dist__manual_replaced_release_20260830_145052`. No supported noninteractive packaged-app smoke exists in this repository; WebEngine executable launch was intentionally not run in this sandbox, so packaged interactive smoke remains not validated here.

## 2026-08-30: Governed Repository-Only Orphan-Record Repair

- Cory supplied factual authority to remove exactly stable matter ID `17bcaa4f-658d-48b0-84e7-01c4411ce135` and time-entry IDs `T_9a77764a3b` / `T_38a43a4197` from the divergent repository snapshot. The configured live/cloud CSPM and Dockets workbooks already excluded them and remained the authoritative reconciled copies. Protected invoice `26-0080` was explicitly outside the repair and remained value-for-value unchanged.
- Read-only preflight proved the exact three-record shape, unbilled state, two-and-only-two dependents, absence of invoice/payment/credit/adjustment/transaction/ledger links, disjoint protected-invoice chain, repository/live classification, live/cloud absence, and historical evidence. No legitimate retirement status could eliminate the required-value integrity error, so the authorized physical removal convention was used.
- Added `scripts/repair_confirmed_orphan_records.py`. It is locked to `data/CSPM.xlsm` and the three stable IDs, defaults to dry-run, requires explicit `--apply`, rejects any mismatch or second destructive execution, reads/hashes live/cloud/historical boundaries without writing them, and contains no confidential names or narratives. It follows the established repository convention: verified backup, isolated candidate, exact-key row removal with table-ref shrink, reopen/semantic/package/control verification, canonical integrity, source-hash guard, same-volume staged `os.replace`, and promoted reopen verification. It is not connected to application startup.
- Exactly two `tblTimeEntries` rows and one `tblMatters` row were removed. All unrelated rows and row styles, financial control totals, protected-invoice rows, formulas, defined names, validations, table headers/styles, sheet order/states, formatting catalogs, and package membership compare unchanged. The macro-enabled content type and all 77 package members remain; no VBA project binary existed before or after.
- Governed evidence is under `backups/CSPM/repository_record_repair_20260830_161006Z/`: `CSPM.before-repair.xlsm`, `candidate/CSPM.xlsm`, and sanitized `repair_audit.json` with rollback instructions. Original and backup SHA-256 are `98FB1AC9176A959A11E1614CCB870BE5EF15327AEB453F74F2EAFA35E4A613B1`; candidate and promoted SHA-256 are `CDCAF48AF137A9A489560EEED61D760E7876390160F6B0D66DF20E9A18B9ACEA`.
- Canonical integrity passed independently against both candidate and promoted repository workbook: **12 tables / 1,537 rows / 0 errors / 7 warnings**. The prior target `missing_required_value` error is absent. The warnings remain exactly four `missing_account_reference` and three `missing_business_unit_reference` issues; none was suppressed or repaired.
- Added `tests/test_repair_confirmed_orphan_records.py`. Its 12 required behaviors execute as **17 passed in 78.93s**: exact-scope deletion, dependency order, unexpected-dependent refusal, six financial-dependency refusal cases, target-error removal, warning retention, protected-invoice preservation, control-total preservation, package/structure preservation, live/cloud target refusal, explicit already-repaired behavior, and sanitized output. Adjacent workbook-integrity/matter/time-entry/archive/deletion regression files passed **17 tests in 9.14s**. Post-promotion disposable-copy reruns passed **3 tests in 19.88s** for exact scope/structure/repeat execution and **9 tests in 47.92s** for exact scope, all six financial-dependency refusals, protected-invoice preservation, and repeat execution after final utility hardening. Python compilation passed. No QML or PowerShell source changed, so QML lint and PowerShell parser checks were not applicable.
- Final fingerprints confirm the four configured live/cloud workbooks and historical evidence are byte-identical to their before state; only `data/CSPM.xlsm` changed. Files changed for this repair: `data/CSPM.xlsm`, `scripts/repair_confirmed_orphan_records.py`, `tests/test_repair_confirmed_orphan_records.py`, `task.md`, and `implementation.md`. Existing unrelated dirty work was preserved. No commit, push, EXE build, publication, custom-fee production fix, or conflict-checking work was performed.
- Next separate high-priority issue: make the Custom Fee workflow idempotent and prevent orphan creation. Conflict checking remains inactive.

## 2026-08-29: Phase 5.5 Missing Matter Number Diagnosis — Superseded by Factual Confirmation

- The canonical `scripts/check_workbook_integrity.ps1` path reproduced one `missing_required_value` error in `tblMatters.MatterNumber`. Exactly one row in the divergent repository snapshot was affected: stable `MatterID` `17bcaa4f-658d-48b0-84e7-01c4411ce135`. The check covered 12 tables / 1,540 rows and retained the seven separate reference warnings (four account-reference and three business-unit-reference warnings).
- Root cause is genuinely missing authoritative data in a sparse legacy/out-of-band matter row, not an incorrect schema/header mapping or checker defect. `schema/workbook_schema.yml`, `domain/schema_constants.py`, and `WorkbookIntegrityService` agree on the column. The required rule predates the row. The row is absent from the August 6 committed workbook revision and first appears in the August 10 committed workbook snapshot. Every inspected repository-supported matter writer, from initial history through current `save_matter_profile` and `_get_or_create_matter`, generates a nonblank number and persists a complete reloadable row.
- The record cannot be repaired deterministically from currently available authoritative data. It has linked time records, so it cannot be discarded as an unused blank row, but it has no matter number, opening date, matter type, timestamps, matching legacy matter-registry row, or exact source-docket match. Generating a number with `General` and today's date would fabricate business facts and alter numbering sequence semantics.
- Added `tests/test_workbook_integrity_matter_number.py`. Synthetic tests prove sanitized detection, the valid case, main matter-profile creation, typed-docket matter creation, reload verification, byte/ZIP/table preservation, coexistence of seven warnings, and the inactive conflict/A08 boundary. Results: focused suite **4 passed in 8.15s**; focused plus adjacent matter/repository suite **20 passed in 17.82s**.
- Diagnosis-time workbook action: none. A before/after SHA-256 comparison around the canonical check was identical; no checkout, backup, save, promotion, or recovery occurred during diagnosis. The then-current result was exit 1 with one error and seven warnings. No warning or template work was attempted.
- Files changed for this diagnosis: `tests/test_workbook_integrity_matter_number.py`, `task.md`, and `implementation.md`. No production Python, QML, PowerShell, schema, template, workbook, payment, invoice, docket, ledger, trust, tax, or conflict code was changed. Conflict checking remains inactive.
- Cory's later factual confirmation established that no `MatterNumber` should exist because the row and its two dependents were abandoned artificial records. The governed repository-only repair above supersedes the earlier one-cell decision path.
...
- `PaymentEntryView.qml` now provides a persistent exact party scope above the existing free-text search: **All open invoices**, **Client**, or **Billing client**. Party choices are derived from the live eligible open-invoice set, and the selected scope is preserved in workspace state.
- The repository payment query accepts `partyType` and `partyValue` and applies a case-insensitive exact identity match before returning rows. It continues to use the canonical A/R calculation and existing broader invoice/client/matter/balance search contract.
- Every open-invoice row now has an **Add Payment** action. `QuickPaymentDialog.qml` opens with the invoice's complete payment context and defaults the payment amount to its exact remaining balance while still allowing partial payment or an adjustment.
- The modal exposes the same governed fields as Payment & Adjustment Entry and posts through the existing asynchronous `postPayment` command. Existing validation enforces date format, positive amounts, balance limits, payment method, deposit account, and adjustment reason. The UI disables competing saves while a post is active.
- A successful quick post closes the modal, preserves the selected party/search filter, refreshes party options and open invoices, updates payment history, and removes a fully settled invoice from the open list. Failures remain in the modal with the backend message.
...
## 2026-09-24: WIP-to-Bill Disbursement Identity & Flat-Fee Entry Repair

- `BillingController._load_unbilled_wip_impl` now canonicalizes disbursement rows and recovers service-client, billing-client, and matter identity from governed legacy aliases, client names, matter ownership, and matter numbers. A disbursement remains eligible WIP based on client-invoice evidence—not supplier-payment status—and no time docket is required for the row to appear.
- `schema_constants.TABLE_ALIASES` now recognizes the historic disbursement headers used by synchronized workbooks, including `Client`, `Sub-Client`, `Matter_ID`/`MatterNumber`, `Bill %`, legacy invoice labels, and created/supplier-reference fields.
- WIP-to-Bill cells now show explicit safe fallbacks for unresolved date/client/billing-client/matter/description fields, so a malformed legacy row cannot present as an amount-only blank row.
- Time Docket Entry now exposes a visible **Flat Fee** action. It opens the existing governed Fee Docket Entry panel and carries the current date, client, matter, and description; the reverse **Time Entry** action returns to timed docketing. This reuses the existing verified fee-entry repository path rather than adding a second fee model.
- Synthetic validation: `test_wip_workbench_performance.py` proves a disbursement-only matter with legacy headers resolves to a searchable canonical client/matter row; `test_fee_docket_workflow_contract.py` protects the cross-workflow flat-fee handoff and nonblank row fallbacks.
- Automated checks: 49 focused A/P, invoice, fee, reversal, and WIP tests passed; Python compilation passed; governed QML lint exited 0 with pre-existing warning-only diagnostics.
- Manual check still required: use synthetic data in the foreground app to confirm WIP row appearance/search, draft creation from a disbursement-only selection, and the Time Entry → Flat Fee handoff. No production workbook was opened or changed during implementation.
- Follow-up diagnosis of the affected live record found a governed A/P bill and expense transaction linked to the correct matter, but both stored `BillClaimPct = 0`; consequently no `Disbursements` row existed. The initial A/P form displayed `100` only as placeholder text while its actual field value was blank, which serialized to zero.
- A/P matter expenses now begin with a real 100% recovery value, display the resulting CAD client-WIP amount, reject blank/invalid/out-of-range percentages, and present an explicit warning dialog before an intentional 0% save. The service independently requires the same non-recoverable acknowledgement so direct callers cannot bypass the guard.
- Existing bills retain an explicit stored zero when opened for editing. Updating a paid bill from zero to a positive recovery percentage now runs the full governed currency preparation and can create the missing disbursement at the correct CAD amount without altering the supplier payment.
- Follow-up validation: Python compilation passed; 57 focused A/P, invoice, fee, reversal, and WIP tests passed; governed QML lint exited 0 with pre-existing warning-only diagnostics. The live workbook was inspected read-only and was not modified.
- The production-shaped disposable-workbook test exposed one additional contract mismatch: `sync_supplier_disbursement` delegated its create path to `create_supplier_disbursement`, whose successful result included the new ID but not `action: created`. The orchestration layer therefore created valid WIP but did not write that ID back to the A/P bill. The repository now supplies a consistent create action, and orchestration defensively persists any returned authoritative disbursement ID unless the result explicitly reports deletion.
- Regression coverage includes the former action-less create contract. A fresh disposable copy of the live workbook then passed the complete repair path: one correctly valued `Unbilled` disbursement was created, its ID exactly matched the persisted A/P `DisbursementID`, and payment plus expense-transaction row counts remained unchanged.
- Validation and release: the focused A/P, billing, invoice, fee, payment, reversal, and WIP gate passed **95 tests**; Python compilation and `git diff --check` passed; governed QML lint remained clean apart from pre-existing warnings. The candidate build passed template-confidentiality checks and produced 4,361 files / 678,819,150 bytes / tree SHA-256 `12DF225210BF27ED84BC5986CC2A6B1389240013C5B86DDD6992F5B3325BB407`. The canonical and installed executables match at SHA-256 `27540E289C17892B182C91CAA988A7CE526C1B737C44230FD106CABB8E42CA23`; the previous package is recoverable at `to_delete/dist__manual_replaced_release_20260924_211156`.
- Live repair: CSPM's exclusive checkout reported an unchanged cloud/local starting point. A separately hash-verified pre-repair package was retained under the local CSPM backup tree. The governed update created exactly one recoverable unbilled disbursement and one corresponding ledger audit row, persisted the disbursement ID on the A/P bill, left bill/payment/expense-transaction counts unchanged, and was returned by the WIP workbench backend at the correct amount with zero client HST. Publish completed with exact local/cloud workbook hashes and released the checkout marker. The installed package subsequently reached input-ready, closed normally through its animated shutdown path, published an unchanged state, and again released the marker.

## 2026-09-26: Invoice Billing-Client Integrity & Audited Correction

- Root cause: draft PDF rendering resolved the work client's billing parent, but ordinary draft finalization independently defaulted Receivables, Invoice Log, and Ledger ownership to the work client unless a joint-retainer snapshot happened to exist. For `26-0095`, this produced a correct Leviathan Private Network PDF and incorrect Concierge Club A/R ownership.
- `InvoiceDraftService.create_draft` now freezes the resolved parent/direct bill-to profile for every draft. Finalization consumes that immutable identity, and safely creates a snapshot for an older draft that lacks one. Work-client identity remains separate in `InvoiceLog.ClientName/SubClient`, `Receivables.WorkClient`, and `Ledger.WorkClient`.
- `InvoiceDraftService.invoice_billing_correction_context` reconstructs the current work/billing identities from linked time, disbursement, matter, A/R, and invoice records, reports a mismatch/recommended parent, and exposes active client choices. Eligibility is limited to one open receivable with zero payments, zero credits, and a positive balance.
- `correct_finalized_invoice_billing_client` repeats the financial gate under the draft lifecycle lock and commits one bulk workbook replacement covering Invoice Log, Receivables, invoice-revenue Ledger rows, and eligible invoice-issuance transactions. It preserves receipt/payment evidence and appends timestamped from/to/reason audit history to `BillToSnapshot`.
- `BillingController` provides background context/correction workers. `InvoiceReversalView.qml` (the Invoice Directory surface) now detects and displays a billing mismatch and provides **Correct Billing Client**, with current work client, current billing client, recommended matter parent, active-client selection, required reason, and explicit scope disclosure.
- No live workbook or invoice was changed during implementation. Sandbox-safe validation: changed Python modules compile; all focused invoice/WIP tests pass (**60 passed**); the required QML lint wrapper exits 0 with warning-only diagnostics; `git diff --check` passes. The unrelated `test_ap_validation_repair.py` file on the September 24 base still calls a nonexistent `FinanceRepo.save_txn`, and the broader collection retains two legacy import-path errors plus one obsolete motion-settings import; these are not part of this billing-client path.
- Rebuild/deployment: the feature was integrated onto the clean September 24 billing/release branch rather than retaining the older stash-preservation branch's embedded conflict markers. The governed `--candidate-validation --validate` build passed template safety checks, promoted `dist\CSPM`, and deployed the identical package to `C:\Programs\CSPM`. Both trees contain **4,275 files / 674,267,194 bytes** with manifest SHA-256 `90DCED418DB835EBD3CA9EEF86E61DBE0BDCC0A0423223AA3386E2D2D1CFF90E`; both executables have SHA-256 `6E3E16F99C47332984AA0CABA63D3E8F2F2669C9C633881E0944D506C0E62C99`. Source, dist, and installed `InvoiceReversalView.qml` hashes match at `2FF6DBBA7259230901870BA6A79987531726490EE33BD7B0DED78314243557FA`. The previous release is recoverable at `to_delete\dist__replaced_release_20260926_134000`.
- Outside-sandbox real WebEngine validation: the installed executable reached `startup-input-ready` in **14.517 seconds**, remained responsive, accepted a normal close, and logged `Shared-data publish on shutdown: Cloud authority and local replica already match.` The manual invoice `26-0095` correction remains intentionally pending so the user can review the detected current/recommended parties and supply the audit reason in-app.
- A follow-up screenshot exposed that `workClientCombo` alone was forced to 36 px despite using the shared floating-label `ModernComboBox`; its **All clients** value was clipped against the allocation grid. The filter row and combo now both use `dialog.fieldHeightPx`, matching the correctly rendered form controls above. The focused Payment Entry/collection contract passes **8 tests**, and governed QML lint exits 0.
- The follow-up candidate-validation build deployed identical **4,275-file / 674,267,532-byte** dist and installed trees (manifest SHA-256 `F3039105E78E2722A007FE928ED759DEF748E8F43F32FDB9B1767BE0B244A4D2`). Both executables have SHA-256 `708DA0868B08DAE503AC2673601270893C9F1E7BC91A76D5C46519BBA9B63EB5`; source/dist/installed dialog QML match at `D2F89927B0715A79D9EEA60387012BCBC021D4A30842ACCE8BF21D21F192FE40`. The prior package is recoverable at `to_delete\dist__replaced_release_20260926_135122`. Outside-sandbox startup reached input-ready in **14.046 seconds** and normal shutdown again confirmed the cloud and local workbook were unchanged.
- Billing Client A/R Collection now treats a click on an invoice row as a quick allocation toggle. An unallocated row fills **Apply ($)** with its full outstanding balance; clicking an allocated row clears it. The selected state uses the semantic accent border/background and a hover tooltip explains the available action. Clicking inside **Apply ($)** still permits direct partial-value editing, and this shortcut never rewrites the independently entered **Amount received ($)**. Sandbox-safe validation passed the focused payment/invoice suite (**25 tests**), the governed QML lint wrapper, and `git diff --check` (line-ending notices only).
- Release/deployment: the governed candidate-validation build passed template-confidentiality checks and deployed identical **4,275-file / 674,269,738-byte** trees to `dist\CSPM` and `C:\Programs\CSPM` (manifest SHA-256 `9B9A633BDF4682D2291997BEBFCC346E84443F0C12BEA0D2248015AFAF83A73D`). Both executables have SHA-256 `242EE2F79926DEFC0B70EF879BC3EE40C2718239098C1196017D89EB54AFEEA0`; source, dist, and installed collection-dialog QML match at `DC309BE84A3B3DF512F9BD9C5A0B885AC37212036659B7DCB1611C17D077A888`. The preceding package is recoverable at `to_delete\dist__replaced_release_20260926_140744`. Outside-sandbox real WebEngine validation reached `startup-input-ready` in **10.240 seconds**, accepted the normal animated close, exited with code 0, and logged that cloud authority and local replica already match.
