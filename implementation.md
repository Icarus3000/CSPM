...
- Added focused repository coverage for one parent transaction/two allocations, A/R/WIP synchronization, pre-write total and party validation, read-only allocated-receipt history, plus QML/controller contract coverage. Sandbox-safe validation passed: Python compilation; **62 focused and adjacent tests**; both changed QML components compiled `Ready`; governed QML lint had no errors (existing `PaymentEntryView.qml` warnings remain); and `git diff --check` passed.
- The supported candidate-validation build produced a complete 4,325-file package at `dist\CSPM`; `CSPM.exe` SHA-256 is `20965F32BC0DBD89B82E87F65AFCA7C0759ECC079D8B723D75BA459FD054A3FD`. That package was deployed through the repository's user-data-preserving deployment function to `C:\Programs\CSPM`; installed and dist executable hashes match, as do source/dist/installed hashes for `PaymentEntryView.qml` and `BillingClientARCollectionDialog.qml`. The prior dist package is recoverable at `to_delete\dist__replaced_release_20260910_100706`. A production-approved build remains intentionally unavailable while the template is `CANDIDATE_AWAITING_CORY_APPROVAL`.
- Outside-sandbox packaged Qt/WebEngine validation passed from both dist and the installed executable. The installed app reached first main-window pixel at 13.422 seconds, remained responsive, and closed through the normal application transition; local/cloud data already matched and no shared checkout marker remains. The exact collection/post interaction remains for foreground manual acceptance against a recoverable test workbook; no financial data was changed during validation.
