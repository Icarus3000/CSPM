...
- The supported candidate-validation build produced a complete 4,325-file package at `dist\CSPM`; `CSPM.exe` SHA-256 is `20965F32BC0DBD89B82E87F65AFCA7C0759ECC079D8B723D75BA459FD054A3FD`. Source and packaged hashes match for `PaymentEntryView.qml` and `BillingClientARCollectionDialog.qml`. The prior package is recoverable at `to_delete\dist__replaced_release_20260910_100706`. A production build remains intentionally unavailable while the template is `CANDIDATE_AWAITING_CORY_APPROVAL`.
- Outside-sandbox packaged Qt/WebEngine validation reached first main-window pixel at 16.529 seconds and completed deferred shell startup without QML errors. The validation process closed through the normal application transition; local/cloud data already matched and no shared checkout marker remains. The exact collection/post interaction remains for foreground manual acceptance against a recoverable test workbook; no financial data was changed during validation.
