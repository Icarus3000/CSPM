    component DataTable: Rectangle {
        ...
        function effectiveColumnWidth(idx, availableWidth) {
            if (idx < 0 || idx >= columnModel.count) return 0
            var col = columnModel.get(idx)
            if (!col || !col.visible) return 0
            
            var storedWidth = col.width
            var minWidth = col.minWidth || 50
            
            if (idx === table.fillColIdx) {
                var taken = 0
                for (var i = 0; i < columnModel.count; i++) {
                    if (i !== idx && columnModel.get(i).visible) {
                        taken += columnModel.get(i).width
                    }
                }
                return Math.max(minWidth, availableWidth - taken)
            }
            return Math.max(minWidth, storedWidth)
        }

        function handleColumnResize(idx, requestedWidth, tableBodyWidth) {
            if (idx < 0 || idx >= columnModel.count) return
            var col = columnModel.get(idx)
            var minWidth = col.minWidth || 50
            var currentWidth = col.width
            var req = Math.max(minWidth, Math.round(Number(requestedWidth)))
            var delta = req - currentWidth
            ...
            if (nextIdx >= 0 && nextIdx !== table.fillColIdx) {
                var nextCol = columnModel.get(nextIdx)
                var nextMin = nextCol.minWidth || 50
                var nextWidth = nextCol.width
                
                var maxGrow = Math.max(0, nextWidth - nextMin)
                var maxShrink = Math.max(0, currentWidth - minWidth)
                
                delta = Math.max(-maxShrink, Math.min(delta, maxGrow))
                if (delta === 0) return
                
                columnModel.setProperty(idx, "width", currentWidth + delta)
                columnModel.setProperty(nextIdx, "width", nextWidth - delta)
            } else {
                var fillMin = 50
                if (table.fillColIdx >= 0) {
                    var fillCol = columnModel.get(table.fillColIdx)
                    if (fillCol) fillMin = fillCol.minWidth || 50
                }
                
                var usedOther = 0
                for (var j = 0; j < columnModel.count; j++) {
                    var c = columnModel.get(j)
                    if (!c || !c.visible || j === table.fillColIdx || j === idx) continue
                    usedOther += c.width
                }
                
                var maxAvailableForIdx = tableBodyWidth - usedOther - fillMin
                req = Math.max(minWidth, Math.min(req, maxAvailableForIdx))
                delta = req - currentWidth
                if (delta === 0) return
                
                columnModel.setProperty(idx, "width", req)
            }
        }
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Rectangle {
                                    ...
                                    width: visible ? table.effectiveColumnWidth(headerCell.index, tableBody.width) : 0
                                        ...
                                        onPositionChanged: function(mouse) {
                                            ...
                                            if (headerCell.manualDragging) {
                                                ...
                                                for (var i = 0; i < columnModel.count; i++) {
                                                    ...
                                                    var cWidth = table.effectiveColumnWidth(i, tableBody.width)
                                    ...
                                    Text {
                                        ...
                                        anchors.rightMargin: (headerCell.model.resizable && headerCell.index !== table.fillColIdx) ? root.tableResizeHandleWidthPx : 8
                                    ...
                                    MouseArea {
                                        ...
                                        onPositionChanged: function(mouse) {
                                            if (resizeHandle.pressed) {
                                                ...
                                                table.handleColumnResize(headerCell.index, req, tableBody.width)
                                                mouse.accepted = true
                                            }
                                        }
                                        onReleased: {
                            ...
                            Row {
                                ...
                                Repeater {
                                    ...
                                    delegate: Rectangle {
                                        ...
                                        width: visible ? table.effectiveColumnWidth(bodyCell.index, tableBody.width) : 0
                                        ...
                                        Text {
                                            ...
                                            anchors.rightMargin: (bodyCell.model.resizable && bodyCell.index !== table.fillColIdx) ? root.tableResizeHandleWidthPx : 8
                                            color: root.inkColor
                                            opacity: 0.92
                                            font.family: visualRules.textFontFamily
                                            font.pixelSize: 12
